            ...
            {menuWithDynamics.map((item) => {
              ...
              return (
                <div key={item.label} className="relative" onMouseEnter={() => setActiveMenu(item.label)} onMouseLeave={() => setActiveMenu(null)}>
                  <Link to={item.path} className={`relative z-10 px-5 py-2.5 fluid-menu-item font-black uppercase tracking-[0.15em] transition-all flex items-center gap-2 rounded-full ${isActive ? 'text-white' : isHovered ? 'text-blue-400' : 'text-slate-400 hover:text-slate-300'}`}>
                    {t(item.label)}
                  ...
                  <AnimatePresence>
                    {item.sub && isHovered && (
                      <motion.div initial={{ opacity: 0, y: 15, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 15, scale: 0.95 }} className="absolute top-full left-1/2 -translate-x-1/2 w-72 bg-[#0f172a] border border-white/10 rounded-[2.5rem] shadow-[0_40px_80px_rgba(0,0,0,0.6)] p-3 mt-4 backdrop-blur-3xl">
...
const SubmenuItem: React.FC<{ item: any; index: number }> = ({ item, index }) => {
  ...
  return (
    <div className="relative" onMouseEnter={() => item.sub && setIsOpen(true)} onMouseLeave={() => setIsOpen(false)}>
      <Link to={item.path} className="flex items-center gap-4 px-5 py-4 text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-white hover:bg-blue-600/20 rounded-2xl transition-all group/sub">
      ...
      <AnimatePresence>
        {item.sub && isOpen && (
          <motion.div initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 10 }} className="absolute left-[102%] top-0 w-64 bg-[#0f172a] border border-white/10 rounded-[2.5rem] shadow-2xl p-3 backdrop-blur-3xl z-[1100]">
            <div className="space-y-1">
