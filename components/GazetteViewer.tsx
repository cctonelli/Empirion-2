...
const getTariffHeatmapColor = (val: number) => {
  if (val >= 100)
    return "bg-[#7f0000] text-red-100 border border-slate-900/40 text-[9px] font-mono";
  if (val >= 50)
    return "bg-[#bf1a1a] text-red-50 border border-slate-900/40 text-[9px] font-mono";
  if (val >= 25)
    return "bg-[#c19114] text-amber-50 border border-slate-900/40 text-[9px] font-mono";
  if (val >= 15)
    return "bg-[#f59e0b] text-[#020617] border border-slate-900/30 text-[9px] font-bold font-mono";
  if (val >= 5)
    return "bg-[#76b014] text-[#020617] border border-slate-900/30 text-[9px] font-bold font-mono";
  if (val >= 1)
    return "bg-[#10b981] text-[#020617] border border-slate-900/30 text-[9px] font-bold font-mono";
...
const GazetteViewer: React.FC<GazetteViewerProps> = ({
  arena,
  aiNews,
  round,
  ...
  const currentMacro = useMemo((): MacroIndicators => {
    ...
    const maxShifts =
      ...
      baseSettings.max_shifts ??
      baseSettings.workforce?.max_shifts ??
      DEFAULT_MACRO.max_shifts;
    const prodHours =
      rules.production_hours_period ??
      rules.workforce?.production_hours_period ??
      baseSettings.production_hours_period ??
      baseSettings.workforce?.production_hours_period ??
      DEFAULT_MACRO.production_hours_period;
  ...
  const chronogramHistory = useMemo(() => {
    ...
    for (let r = 0; r <= totalMax; r++) {
      ...
      list.push({
        ...
        EUR: activeMacro.EUR ?? 5.6,
        GBP: activeMacro.GBP ?? 6.5,
      ...
      yaxis: {
        labels: {
          ...
          formatter: (val: number) =>
            val >= 1 ? val.toFixed(1) : val.toFixed(5),
        },
      },
      tooltip: {
        theme: "dark",
  ...
  const fetchMarketIntelligence = async () => {
    ...
    try {
        ...
        if (teamsWithKpis && teamsWithKpis.length > 0) {
          data = teamsWithKpis.map((t: any) => ({
            ...
            equity:
              t.equity ||
              t.kpis?.equity ||
              (arena.starting_mode === "start_from_zero"
                ? (arena.config?.caixa_inicial ?? 111163.54)
                : 7252171.74),
            team: { name: t.name },
            kpis: t.kpis || {
      ...
      console.error("Erro ao coletar dados competitivos:", err);
    } finally {
  ...
  useEffect(() => {
    if (activeTab === "market" || activeTab === "audit")
      ...
      try {
          ...
          if (teamsWithKpis && teamsWithKpis.length > 0) {
            teamsData = teamsWithKpis.map((t: any) => ({
              ...
              kpis: t.kpis || {
                ...
                statements: INITIAL_FINANCIAL_TREE,
                share_price:
                  (t.equity || t.kpis?.equity || defaultEquity) / 72000,
                rating: "AAA",
              },
            }));
          }
        ...
        if (teamsData && teamsData.length > 0) {
          const news = await generateDynamicMarketNews(
            ...
            teamsData.map((t) => ({
              name: t.team?.name,
              market_share: t.kpis?.market_share,
              equity: t.equity,
              rating: t.kpis?.rating,
            })),
            currentMacro,
            arena.transparency_level || "medium",
            arena.gazeta_mode || "anonymous",
          );
          setDynamicNews(news);
        } else {
          setDynamicNews(
            "Nenhuma atividade comercial registrada na auditoria do round anterior.",
          );
        }
      } catch (err) {
        setDynamicNews(
          "Instabilidade temporária identificada na frequência de rádio da Oracle Gazette. Os dados estruturados macroeconômicos e o radar concorrencial permanecem plenamente operacionais nas abas ao lado.",
        );
      } finally {
    ...
    >
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-1 justify-center md:justify-start">
              ...
              <span className="text-[10px] text-slate-400 font-normal tracking-wide flex items-center gap-1">
                <Globe size={11} className="text-slate-500" /> Notícias de
        ...
        <div className="flex flex-wrap items-center gap-4 w-full lg:w-auto justify-center lg:justify-end">
          <nav className="flex items-center gap-1 p-1 bg-slate-900/90 rounded-lg border border-slate-800">
            <SectionTab
              ...
              label="Caderno Principal"
            />
            <SectionTab
              active={activeTab === "market"}
              onClick={() => setActiveTab("market")}
              icon={<BarChart3 size={13} />}
              label="Radar Concorrencial"
            />
            <SectionTab
              active={activeTab === "macro"}
              onClick={() => setActiveTab("macro")}
              icon={<Zap size={13} />}
              label="Macroeconomia & Setorial"
            />
            <SectionTab
      ...
      <div className="bg-[#0b1320] border-b border-slate-800/80 px-6 py-2.5 flex flex-wrap items-center justify-between text-xs font-mono text-slate-400 gap-3 shrink-0">
        <div className="flex items-center gap-4">
          ...
          <span>•</span>
          <span className="uppercase text-[11px] font-semibold text-amber-500 bg-amber-500/5 px-2 py-0.5 border border-amber-500/10">
            Simulação Ativa: {arena.name}
          </span>
        </div>
        <div className="flex items-center gap-4">
          <span>
            ...
            <span className="text-sky-400 font-bold">
              {translateTransparency(arena.transparency_level)}
            </span>
          </span>
          ...
          <span>
            ...
            <span className="text-indigo-400 font-bold">
              {(arena.gazeta_mode || "anonymous").toUpperCase()}
            </span>
          </span>
        </div>
      </div>
        ...
        <AnimatePresence mode="wait">
            ...
            >
              ...
              {currentMacro.is_black_swan && (
                <div className="p-6 bg-red-950/25 border-y-2 lg:border-2 border-red-500/40 rounded-xl relative overflow-hidden group shadow-lg">
                  <div className="absolute -right-8 -top-8 opacity-5 group-hover:rotate-12 transition-transform text-red-500">
                  ...
                  <div className="relative z-10 space-y-2">
                    <div className="flex items-center gap-2.5 text-red-400 font-mono text-xs tracking-wider">
                      ...
                      <span className="font-extrabold uppercase animate-pulse">
                        SUPLEMENTO DE URGÊNCIA • EXTRA EDIÇÃO
                      </span>
                    </div>
                    <h3 className="text-2xl lg:text-3xl font-extrabold text-white uppercase font-serif tracking-tight">
                      {currentMacro.black_swan_title}
                    </h3>
                    <p className="text-red-100 text-xs lg:text-sm leading-relaxed max-w-5xl italic">
                      {currentMacro.black_swan_description}
                    </p>
                  </div>
                </div>
              )}
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 w-full">
                ...
                <div className="lg:col-span-8 bg-slate-950/45 border border-slate-800/80 rounded-2xl p-6 lg:p-8 shadow-inner relative overflow-hidden min-h-[500px]">
                  <div className="border-b border-slate-800 pb-3 mb-6 flex items-center justify-between">
                    <span className="text-[11px] font-black text-amber-500 uppercase tracking-widest italic flex items-center gap-2">
                      <Sparkles size={12} /> Editorial Strategos
                    </span>
                    ...
                    <div className="flex flex-col items-center justify-center gap-4 py-24">
                      ...
                      <div className="text-center">
                        <span className="text-xs font-bold text-slate-400 uppercase tracking-widest block">
                          Transmissão em processamento...
                        </span>
                        <span className="text-[10px] text-slate-500 font-mono block mt-1">
                          Auditando balancetes e ajustando indexadores setoriais
                        </span>
                      </div>
                    </div>
                  ) : (
                ...
                <div className="lg:col-span-4 space-y-6">
                  ...
                  <div className="bg-[#020617] p-5 rounded-2xl border border-slate-800 shadow-xl space-y-4">
                    ...
                    <div className="space-y-3.5">
                      ...
                      <MarketBriefItem
                        ...
                        status={
                          currentMacro.inflation_rate > 3.5 ? "up" : "stable"
                        }
                      />
                      <MarketBriefItem
                  ...
                  <div className="bg-[#020617] p-5 rounded-2xl border border-slate-800 shadow-xl space-y-4">
                    <h5 className="text-xs font-extrabold uppercase tracking-widest text-slate-400 font-mono border-b border-slate-800 pb-2 flex items-center gap-2">
                      <Coins size={13} className="text-sky-400" /> Cotações
                      Cambiais (vs. Moeda-Padrão)
                    </h5>
                    ...
                    <p className="text-[10px] text-slate-500 font-mono leading-relaxed">
                      Evolução cambial do campeonato para BRL (azul), USD
                      (verde), EUR (laranja), GBP (turquesa), CNY (rosa) e BTC
                      (roxo).
                    </p>
                  </div>
                  ...
                  <div className="bg-[#020617] p-5 rounded-2xl border border-slate-800 shadow-xl space-y-4">
                    <div className="border-b border-slate-800 pb-2 flex items-center justify-between">
                      ...
                      <span className="text-[9px] text-[#10b981] font-mono uppercase bg-slate-950 px-1.5 py-0.5 rounded border border-slate-900 font-black">
                        Heatmap
                      </span>
                    </div>
                      ...
                      <table className="w-full border-collapse">
                        ...
                        <tbody>
                          {regionsTariffs.map((region, i) => (
                            <tr
                              key={i}
                              className="border-b border-slate-900/50 hover:bg-slate-900/10 border-collapse"
                            >
                    ...
                    <div className="flex flex-wrap gap-x-2 gap-y-1 pt-1 border-t border-slate-900/80 text-[8px] font-mono">
                      <span className="text-slate-400 uppercase tracking-wider block w-full mb-0.5">
                        Legenda Tarifária:
                      </span>
                      <div className="flex flex-wrap gap-1 w-full">
                        <span className="flex items-center gap-0.5">
                          <span className="w-1.5 h-1.5 rounded-sm bg-[#2e7d17]"></span>{" "}
                          0%
                        </span>
                        <span className="flex items-center gap-0.5">
                        ...
                        <span className="flex items-center gap-0.5">
                          <span className="w-1.5 h-1.5 rounded-sm bg-[#76b014]"></span>{" "}
                          5%-14%
                        </span>
                        <span className="flex items-center gap-0.5">
                        ...
                        <span className="flex items-center gap-0.5">
                          <span className="w-1.5 h-1.5 rounded-sm bg-[#c19114]"></span>{" "}
                          25%-49%
                        </span>
                        <span className="flex items-center gap-0.5">
                          <span className="w-1.5 h-1.5 rounded-sm bg-[#bf1a1a]"></span>{" "}
                          50%-99%
                        </span>
                        <span className="flex items-center gap-0.5">
                          <span className="w-1.5 h-1.5 rounded-sm bg-[#7f0000]"></span>{" "}
                          100%+
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="bg-amber-950/15 border border-amber-500/25 p-5 rounded-2xl space-y-3 shadow-md">
                      <div className="flex items-center gap-2 text-amber-400 mb-1">
                        ...
                        <h5 className="font-bold text-xs uppercase tracking-wider font-mono">
                          Nota de Orientação P0
                        </h5>
                      </div>
                      <p className="text-slate-300 text-[11.5px] leading-relaxed">
                        Todas as corporações iniciaram com um uso de Empréstimo
                        Compulsório transitório no Ciclo P0 de modo a sustentar
                        a governança primária e liquidez dos caixas das unidades
                        industriais. Realize projeções financeiras rutilantes
                        para solvência imediata.
                      </p>
                    </div>
                  )}
                </div>
          ...
          {activeTab === "market" && (
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="space-y-6 w-full max-w-7xl mx-auto font-sans"
            >
              ...
              <div className="bg-indigo-950/10 border border-indigo-500/20 p-6 rounded-2xl flex flex-col md:flex-row items-center justify-between shadow-lg gap-4">
                <div className="space-y-1 text-center md:text-left">
                  <h3 className="text-xl lg:text-2xl font-bold font-serif text-white tracking-tight">
                    Monitor de Unidades Compartilhadas
                  </h3>
                  <p className="text-[10px] text-indigo-300/80 font-bold uppercase tracking-widest font-mono">
                    Nível de Segurança de Dados:{" "}
                    {translateTransparency(arena.transparency_level)} (Nível{" "}
                    {arena.transparency_level || "Médio"})
                  </p>
                </div>
                ...
                <table className="w-full text-left min-w-[900px] border-collapse">
                  <thead>
                    <tr className="text-[9px] font-bold uppercase text-slate-400 tracking-wider bg-slate-900 border-b border-slate-800 select-none">
                      <th className="p-4 border-r border-slate-800/60 font-mono">
                        Unidade Industriais (Strategos Competitors)
                      </th>
                  ...
                  <tbody className="divide-y divide-slate-800/70 font-mono text-xs">
                    {loadingCompetitors ? (
                      <tr>
                        <td
                          colSpan={isFullTransparency ? 8 : 7}
                          className="p-16 text-center text-slate-500 italic uppercase font-bold text-xs"
                        >
                          <Loader2
                            className="animate-spin mx-auto mb-3 text-amber-500"
                            size={24}
                          />{" "}
                          Sincronizando dados competitivos e indexadores...
                        </td>
                      </tr>
                      ...
                      <tr>
                        <td
                          colSpan={isFullTransparency ? 8 : 7}
                          className="p-16 text-center text-slate-500 italic uppercase"
                        >
                          Aguardando encerramento de rodada competitiva para
                          consolidação dos balancetes de mercado.
                        </td>
                      </tr>
                    ) : (
                      competitors.map((t, i) => (
                        ...
                        >
                          <td className="p-4 border-r border-slate-800/60">
                            <div className="flex items-center gap-3">
                              <div className="w-7 h-7 bg-slate-800 rounded flex items-center justify-center text-slate-400 font-extrabold text-xs font-mono">
                                {i + 1}
                              </div>
                              <span className="text-sm font-bold text-slate-200 uppercase tracking-tight">
                                {isIdentified || userRole === "tutor"
                                  ? t.team?.name
                                  : `Unidade Alpha-${t.team_id.slice(0, 4)}`}
                              </span>
                            </div>
                          </td>
                          <td className="p-4 text-center text-amber-500 font-extrabold text-lg">
                            {(t.kpis?.market_share || 0).toFixed(1)}%
                          </td>
                          <td className="p-4 text-center text-slate-300 font-semibold">
                            $ {t.equity.toLocaleString()}
                          </td>
                          <td className="p-4 text-center text-emerald-400 font-semibold">
                            ${" "}
                            {(
                              t.kpis?.statements?.dre?.revenue || 0
                            ).toLocaleString()}
                          </td>
                          <td className="p-4 text-center text-sky-400 font-semibold">
                            ${" "}
                            {(
                              t.kpis?.statements?.dre?.net_profit || 0
                            ).toLocaleString()}
                          </td>
                          <td className="p-4 text-center text-amber-400 font-bold text-base">
                            ${" "}
                            {(t.kpis?.share_price || t.equity / 72000).toFixed(
                              2,
                            )}
                          </td>
                          <td className="p-4 text-center">
                            <span
                              className={`px-2 py-0.5 rounded text-[10px] font-bold border ${getLevelColor(t.kpis?.rating)}`}
                            >
                              {t.kpis?.rating || "N/A"}
                            </span>
                          </td>
                          {isFullTransparency && (
                            <td className="p-4 text-center">
                              <span className="text-xs font-semibold text-blue-400">
                                ${" "}
                                {t.kpis?.average_price_sold
                                  ? t.kpis.average_price_sold.toFixed(2)
                                  : "N/A"}
                              </span>
                            </td>
                          )}
            ...
            >
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-3.5">
                <MacroLedgerBox
                  ...
                  val={`${currentMacro.ice}%`}
                  icon={<Activity size={18} className="text-emerald-400" />}
                  desc="Como o setor vê o mercado"
                />
                <MacroLedgerBox
                  label="Inflação Setorial"
                  val={`${currentMacro.inflation_rate}%`}
                  icon={<Flame size={18} className="text-red-400" />}
                  desc="Flutuação geral de preços"
                />
                <MacroLedgerBox
                  label="Juros/Spread Bancários"
                  val={`${currentMacro.interest_rate_tr}%`}
                  icon={<Landmark size={18} className="text-blue-400" />}
                  desc="Financiamentos e Crédito"
                />
                <MacroLedgerBox
                  label="Inadimplência (PECLD)"
                  val={`${currentMacro.customer_default_rate}%`}
                  icon={<ShieldAlert size={18} className="text-orange-400" />}
                  desc="Risco de venda a prazo"
                />
                <MacroLedgerBox
                  label="Juros s/ Empréstimo Compulsório"
                  val={`${currentMacro.compulsory_loan_agio}%`}
                  icon={<ShieldAlert size={18} className="text-red-400" />}
                  desc="Risco de falta de Liquidez"
                />
                <MacroLedgerBox
                  label="Ágio s/ Compras Extras"
                  val={`${currentMacro.special_purchase_premium}%`}
                  icon={<ShieldAlert size={18} className="text-orange-400" />}
                  desc="Risco de compra errada"
                />
                <MacroLedgerBox
                  label="Juros Fornecedor"
                  val={`${currentMacro.supplier_interest}%`}
                  icon={<Truck size={18} className="text-amber-400" />}
                  desc="Acréscimos em compra prazo"
                />
                <MacroLedgerBox
                  label="Rendimento de Aplicação"
                  val={`${currentMacro.investment_return_rate}%`}
                  icon={<TrendingUp size={18} className="text-indigo-400" />}
                  desc="Rendimento do caixa parado"
                />
                <MacroLedgerBox
                  label="IVA s/ Compras"
                  val={`${currentMacro.vat_purchases_rate}%`}
                  icon={<Scale size={18} className="text-teal-400" />}
                  desc="Imposto recuperável de MP"
                />
                <MacroLedgerBox
                  label="IVA s/ Vendas"
                  val={`${currentMacro.vat_sales_rate}%`}
                  icon={<Scale size={18} className="text-rose-400" />}
                  desc="Imposto sobre faturamento"
                />
                <MacroLedgerBox
                  label="Imposto Renda (IR)"
                  val={`${currentMacro.tax_rate_ir}%`}
                  icon={<FileText size={18} className="text-amber-500" />}
                  desc="Tributação sobre lucro operacional"
                />
                <MacroLedgerBox
                  label="Encargos Sociais"
                  val={`${currentMacro.social_charges}%`}
                  icon={<Users size={18} className="text-slate-400" />}
                  desc="Adicional ao salário fabril"
                />
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 pt-2 w-full">
                <div className="bg-[#020617] p-6 rounded-2xl border border-slate-800 shadow-xl space-y-4">
                  <div className="border-b border-slate-800 pb-3 flex items-center gap-2.5">
                    ...
                    <h4 className="text-sm font-bold uppercase tracking-wider text-slate-300 font-mono">
                      Preço Base e Reajustes de Insumos
                    </h4>
                  </div>
                ...
                <div className="bg-[#020617] p-6 rounded-2xl border border-slate-800 shadow-xl space-y-4">
                  <div className="border-b border-slate-800 pb-3 flex items-center gap-2.5">
                    ...
                    <h4 className="text-sm font-bold uppercase tracking-wider text-slate-300 font-mono">
                      Despesas Setoriais de Estocagem (unidade/período)
                    </h4>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full">
                    <CostLedgerItem
              ...
              {/* Seção: Condições Comerciais e Logísticas por Região */}
          <div className="bg-[#020617] p-6 rounded-2xl border border-slate-800 shadow-xl space-y-4 w-full">
            <div className="border-b border-slate-800 pb-3 flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                <MapPin className="text-amber-500" size={18} />
                <h4 className="text-sm font-bold uppercase tracking-wider text-slate-300 font-mono">
                  Condições Comerciais e Logísticas por Região (Vigentes em R-
                  {round})
                </h4>
              </div>
              <span className="text-[10px] text-slate-400 font-mono bg-slate-950 px-2 py-0.5 rounded border border-slate-900">
                {regionConfigs.length}{" "}
                {regionConfigs.length === 1 ? "Região Ativa" : "Regiões Ativas"}
              </span>
            </div>

            {regionConfigs.length === 0 ? (
              <div className="text-center p-8 text-slate-500 italic font-mono text-xs uppercase">
                Nenhuma região ativa parametrizada para este round.
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 w-full">
                {regionConfigs.map((region: any, i: number) => {
                  const tariffKey = (() => {
                    switch (Number(region.id)) {
                      case 1:
                        return "import_tariff_brazil";
                      case 2:
                        return "import_tariff_usa";
                      case 3:
                        return "import_tariff_euro";
                      case 4:
                        return "import_tariff_uk";
                      case 5:
                        return "import_tariff_china";
                      case 6:
                        return "import_tariff_btc";
                      default:
                        return "";
                    }
                  })();
                  ...
                  return (
                    <div
                      key={region.id || i}
                      className="bg-slate-950/40 p-4 rounded-xl border border-slate-800 hover:border-slate-700 transition-all space-y-3.5"
                    >
                      <div className="flex items-center justify-between border-b border-slate-900 pb-2">
                        <div className="flex items-center gap-1.5 min-w-0">
                          <span className="w-2 h-2 rounded-full bg-emerald-500 shrink-0 animate-pulse"></span>
                          <span className="text-xs font-bold text-slate-200 truncate uppercase font-mono tracking-wider">
                            {region.name}
                          </span>
                        </div>
                        <span className="text-[9px] font-black font-mono px-1.5 py-0.5 rounded bg-amber-500/10 text-amber-400 border border-amber-500/20">
                          ID: {region.id}
                        </span>
                      </div>

                      <div className="grid grid-cols-2 gap-2 text-xs font-mono">
                        <div className="bg-[#020617] p-2.5 rounded-lg border border-slate-900/60 flex flex-col justify-between">
                          <span className="text-[9px] text-slate-500 uppercase tracking-wider block mb-1">
                            Moeda Local
                          </span>
                          <span className="text-xs font-bold text-slate-300 flex items-center gap-1 truncate font-sans">
                            <Coins
                              size={11}
                        ...
                        <div className="bg-[#020617] p-2.5 rounded-lg border border-slate-900/60 flex flex-col justify-between col-span-2">
                          <span className="text-[9px] text-slate-500 uppercase tracking-wider block mb-1">
                            Tarifa Alfandegária (Exportação)
                          </span>
                          <span
                            className={`text-xs font-bold flex items-center gap-1 truncate font-sans ${tariffVal > 0 ? "text-red-400 font-extrabold" : "text-emerald-400"}`}
                          >
                            <Scale size={11} className="shrink-0" /> {tariffVal}
                            %
                          </span>
                        </div>

                        {region.suggested_price !== undefined && (
                          <div className="bg-[#020617] p-2.5 rounded-lg border border-slate-900/60 flex flex-col justify-between col-span-2">
                            <span className="text-[9px] text-slate-500 uppercase tracking-wider block mb-1">
                              Preço Consumidor Sugerido
                            </span>
                            <span className="text-xs font-bold text-amber-500 font-sans truncate">
                              {formatCurrency(
                                region.suggested_price,
                                rCurrency,
                              )}
                            </span>
                          </div>
            ...
            >
              ...
              <div className="bg-[#020617] p-6 lg:p-8 rounded-2xl border border-slate-800 shadow-xl text-center space-y-6">
                ...
                <div className="space-y-2">
                  <div className="w-12 h-12 bg-slate-900 border border-slate-800 hover:border-slate-700 rounded-xl flex items-center justify-center text-amber-500 mx-auto transition-all shadow-inner">
                    <User size={22} />
                  </div>
                ...
                {(userRole === "tutor" || isFullTransparency) &&
                competitors.length > 0 ? (
                  <div className="border border-slate-900 bg-slate-950 p-4 rounded-xl max-w-3xl mx-auto space-y-3">
                    <span className="text-[10px] text-slate-400 font-extrabold font-mono uppercase block">
                      Selecione uma de suas concorrentes para o dossiê:
                    </span>
                  ...
                  userRole !== "tutor" && (
                    <div className="text-[10px] text-slate-500 font-mono italic max-w-lg mx-auto bg-slate-950/40 p-3 rounded border border-slate-900/60 leading-relaxed">
                      *Apenas o seu próprio log corporativo está exposto devido
                      à lei de sigilo concorrencial ativa para este campeonato.
                    </div>
                  )
                )}
                ...
                {activeTeam ? (
                  <div className="space-y-6">
                    <div className="space-y-1">
                      <h3 className="text-2xl lg:text-3xl font-extrabold font-serif text-white uppercase italic tracking-tight">
                        RELATÓRIO: {activeTeam.name}
                      </h3>
                      <p className="text-[10px] font-mono font-bold text-slate-500 uppercase tracking-widest">
                        Compilação Oficial Fiscal Referente ao Ciclo P-
                        {round - 1}
                      </p>
                    </div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 w-full">
                      <RelatoryKpiCard
                        label="Valor da Ação"
                        val={`$ ${(activeTeam.kpis?.share_price || activeTeam.equity / 72000).toFixed(2)}`}
                        icon={<Coins className="text-amber-400" size={16} />}
                      />
                      <RelatoryKpiCard
                        label="Patrimônio Líquido"
                        val={`$ ${activeTeam.equity.toLocaleString()}`}
                        icon={
                      ...
                      <RelatoryKpiCard
                        label="Market Share"
                        val={`${(activeTeam.kpis?.market_share || 0).toFixed(1)}%`}
                        icon={<Target className="text-orange-500" size={16} />}
                      />
                      <RelatoryKpiCard
                        label="Rating de Crédito"
                        val={activeTeam.kpis?.rating || "AAA"}
                  ...
                  <div className="py-20 text-slate-500 text-xs font-mono font-bold uppercase italic tracking-widest">
                    Selecione um painel de unidade industrial ativo de modo a
                    visualizar as balanças fiscais consolidadas.
                  </div>
                )}
                  ...
                  <div className="mt-8 p-6 bg-red-950/20 border border-red-500/20 rounded-xl text-left space-y-3">
                    <div className="flex items-center gap-2 text-red-400">
                      ...
                      <h4 className="text-xs font-bold uppercase font-mono tracking-wider">
                        Anotação da Mesa de Auditoria (Strategos Fiscal Board)
                      </h4>
                    </div>
                    <p className="text-slate-300 text-xs leading-relaxed font-serif italic">
                      Ao iniciar a rodada R1, todas as corporações registraram o
                      recebimento de **$ 1.372.362,00** à título de **Uso de
                      Empréstimo Compulsório** no fechamento do P0. Isso evitou
                      a insolvência imediata, sendo necessário quitá-lo
                      integralmente no encerramento do Ciclo R1. Lembre-se que
                      incidem encargos de comissão setorial.
                    </p>
                  </div>
                )}
              </div>
      ...
      <footer className="px-6 py-3.5 bg-[#020617] border-t border-slate-800/80 flex flex-col sm:flex-row justify-between items-center opacity-75 shrink-0 w-full text-[10px] font-mono text-slate-500 gap-4">
        <span className="tracking-[0.4em] uppercase font-bold text-center sm:text-left text-[9px]">
          © Strategos News Network Co. • Protocolo Oracle v18.1 Gold High
          Fidelity
        </span>
        {activeTeam && (
...
const MarketBriefItem: React.FC<MarketBriefItemProps> = ({
  title,
  desc,
  status,
}) => (
  <div className="p-3 bg-slate-950 border border-slate-900 rounded-xl hover:border-slate-800 transition-colors flex gap-3 text-left">
    ...
    <div className="space-y-0.5">
      <h5 className="text-[11.5px] font-bold text-slate-200 uppercase tracking-tight">
        {title}
      </h5>
...
const MacroLedgerBox: React.FC<MacroLedgerBoxProps> = ({
  label,
  val,
  icon,
  desc,
  ...
  <div className="bg-[#020617] border border-slate-800/80 p-4 rounded-xl flex flex-col gap-2 shadow-lg hover:border-slate-700/80 transition-colors">
    <div className="flex items-center justify-between w-full">
      <span className="block text-[8px] font-extrabold text-slate-500 uppercase tracking-widest italic font-mono">
        {label}
      </span>
      <div className="p-1 bg-slate-900 rounded border border-slate-800 text-slate-400">
        {icon}
      </div>
    </div>
    <div>
      <span className="text-xl lg:text-2xl font-black text-amber-500 italic tracking-tight font-sans leading-none">
        {val}
      </span>
      <span className="block text-[9px] text-slate-500 mt-1 leading-tight font-mono">
        {desc}
      </span>
    </div>
  </div>
...
const CostLedgerItem: React.FC<CostLedgerItemProps> = ({
  label,
  val,
  icon,
  currency,
...
const RelatoryKpiCard: React.FC<RelatoryKpiCardProps> = ({
  label,
  val,
  icon,
}) => (
  <div className="p-4 bg-slate-950 rounded-xl border border-slate-900 flex flex-col items-center gap-2 hover:border-amber-500/25 transition-all text-center">
    <div className="p-2 bg-slate-900 rounded-lg text-slate-300 shadow-inner">
      {icon}
    </div>
    <div>
      <span className="block text-[8px] font-extrabold text-slate-500 uppercase tracking-widest mb-1 font-mono">
        {label}
      </span>
...
const getLevelColor = (rating?: string) => {
  switch (rating) {
    ...
    case "AA":
      return "bg-emerald-600/10 border-emerald-500/20 text-emerald-500";
    case "A":
      return "bg-teal-500/15 border-teal-500/30 text-teal-400";
    case "BBB":
      return "bg-blue-500/15 border-blue-500/30 text-blue-400";
    case "BB":
      return "bg-blue-600/10 border-blue-500/20 text-blue-500";
    case "B":
      return "bg-amber-600/15 border-amber-500/30 text-amber-500";
    case "CCC":
