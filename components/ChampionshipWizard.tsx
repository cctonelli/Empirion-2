...
const ChampionshipWizard: React.FC<{ onComplete: () => void }> = ({ onComplete }) => {
  ...
  return (
    <div className="max-w-7xl mx-auto animate-in slide-in-from-bottom-8 duration-500 pb-20">
      ...
      <div className="flex items-center justify-between mb-12 px-10">
        {[1,2,3,4,5,6,7,8].map(s => (
          <div key={s} className="flex flex-col items-center gap-3">
             <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-black transition-all ${step >= s ? 'bg-orange-600 text-white shadow-lg' : 'bg-slate-200 text-slate-400'}`}>
                {s}
             </div>
      ...
      <div className="bg-white rounded-[4rem] shadow-2xl border border-slate-100 overflow-hidden min-h-[750px] flex flex-col">
        <div className="flex-1 p-12 md:p-16 overflow-y-auto">
          ...
          {step === 1 && (
            <div className="space-y-12 animate-in fade-in duration-500 text-center">
               <div className="space-y-4">
                  <h2 className="text-4xl font-black text-slate-900 tracking-tighter uppercase italic">1. Identidade do Ecossistema</h2>
                  <p className="text-slate-500 font-medium">Nomeie sua arena e defina o horizonte temporal.</p>
               </div>
               <div className="grid grid-cols-1 md:grid-cols-2 gap-10 text-left">
                  ...
                  <div className="space-y-3">
                     <label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Ramo Operacional</label>
                     <select className="w-full p-6 bg-slate-50 border border-slate-200 rounded-[1.5rem] font-bold" value={formData.branch} onChange={e => setFormData({...formData, branch: e.target.value as any})}>
                        {Object.entries(BRANCH_CONFIGS).map(([k, v]) => <option key={k} value={k}>{v.label}</option>)}
                     </select>
                  </div>
                  ...
                  <div className="space-y-3">
                     ...
                     <select className="w-full p-6 bg-slate-50 border border-slate-200 rounded-[1.5rem] font-bold" value={formData.transparency} onChange={e => setFormData({...formData, transparency: e.target.value as any})}>
                        <option value="low">Baixa (TSR apenas)</option>
                        <option value="medium">Média (TSR + Vendas)</option>
                        <option value="high">Alta (DRE Público)</option>
                        <option value="full">Auditada (Balanço Aberto)</option>
                     </select>
                  </div>
               </div>
            </div>
          ...
          {step === 2 && (
            <div className="space-y-10 animate-in fade-in duration-500">
               <h2 className="text-3xl font-black text-slate-900 text-center uppercase italic">2. Seleção de Motor Engine</h2>
               <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  {CHAMPIONSHIP_TEMPLATES.map(t => (
                    <button key={t.id} onClick={() => setFormData({...formData, templateId: t.id})} className={`p-10 border-4 rounded-[3.5rem] text-left transition-all group ${formData.templateId === t.id ? 'border-orange-600 bg-orange-50/20 shadow-2xl' : 'border-slate-50 hover:border-slate-100'}`}>
                       <div className="p-4 bg-white rounded-2xl w-fit mb-6 shadow-sm text-2xl">{BRANCH_CONFIGS[t.branch].icon}</div>
                       <h4 className="text-xl font-black uppercase italic">{t.name}</h4>
                       <p className="text-[10px] text-slate-500 font-medium mt-4">{t.description}</p>
                    </button>
                  ))}
                  <button onClick={() => setFormData({...formData, templateId: 'custom'})} className={`p-10 border-4 border-dashed rounded-[3.5rem] text-center transition-all ${formData.templateId === 'custom' ? 'border-orange-600 bg-orange-50/20' : 'border-slate-100 hover:bg-slate-50'}`}>
                     <div className="p-5 bg-slate-900 text-white rounded-full w-fit mx-auto mb-6"><Plus size={32}/></div>
                     <h4 className="text-xl font-black uppercase italic">Configuração do Zero</h4>
                  </button>
               </div>
            </div>
          )}
          ...
          {step === 3 && (
            <div className="space-y-10 animate-in fade-in duration-500">
               <h2 className="text-3xl font-black text-slate-900 text-center uppercase italic">3. Tabela Regional de Demanda</h2>
                     ...
                     <tbody className="divide-y divide-slate-100">
                        {regions.map((reg, idx) => (
                          <tr key={reg.id} className="hover:bg-slate-50 transition-colors">
                             <td className="p-6 font-black uppercase italic"><input className="bg-transparent border-b border-transparent focus:border-blue-400 outline-none w-full" value={reg.name} onChange={e => { const nr = [...regions]; nr[idx].name = e.target.value; setRegions(nr); }} /></td>
                             <td className="p-6"><input type="number" className="bg-slate-100 p-2 rounded-lg w-24" value={reg.demandTotal} onChange={e => { const nr = [...regions]; nr[idx].demandTotal = Number(e.target.value); setRegions(nr); }} /></td>
                             <td className="p-6 font-mono text-blue-600">{reg.initialMarketShare}%</td>
                             <td className="p-6"><input type="number" className="bg-slate-100 p-2 rounded-lg w-24" value={reg.initialPrice} onChange={e => { const nr = [...regions]; nr[idx].initialPrice = Number(e.target.value); setRegions(nr); }} /></td>
                             <td className="p-6"><button onClick={() => setRegions(regions.filter(r => r.id !== reg.id))} className="text-rose-400 hover:text-rose-600"><Trash2 size={16}/></button></td>
                          </tr>
                        ))}
                     </tbody>
                  </table>
               ...
               <div className="flex justify-between items-center">
                  <button onClick={() => setRegions([...regions, { id: Date.now().toString(), name: 'Nova Região', demandTotal: 100000, initialMarketShare: 0, initialPrice: 1000 }])} className="px-8 py-3 bg-slate-900 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest flex items-center gap-2"><Plus size={16}/> Adicionar Região</button>
                  <button onClick={() => setRegions(regions.map(r => ({ ...r, initialMarketShare: Number((100 / regions.length).toFixed(2)) })))} className="px-8 py-3 border border-slate-200 rounded-2xl font-black text-[10px] uppercase text-slate-500 hover:bg-slate-50">Equilibrar Vendas Relativas (%)</button>
               </div>
            </div>
          )}
          ...
          {step === 4 && (
            <div className="space-y-10 animate-in fade-in duration-500">
               <h2 className="text-3xl font-black text-slate-900 text-center uppercase italic">4. Indicadores Macroeconômicos</h2>
            ...
            <div className="space-y-12 animate-in fade-in duration-500 text-center">
               ...
               <div className="max-w-md mx-auto space-y-10">
                  <div className="p-10 bg-slate-50 border border-slate-200 rounded-[4rem] space-y-8">
                     ...
                     <div className="space-y-4">
                        <div className="flex justify-between items-center">
                           <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest flex items-center gap-2"><Bot size={16}/> Quantos BOTS?</label>
          ...
          {step === 8 && (
            <div className="text-center space-y-12 animate-in zoom-in-95 duration-700 py-10">
               <div className="relative inline-block">
                  <div className="w-32 h-32 bg-slate-900 text-white rounded-[3.5rem] flex items-center justify-center mx-auto shadow-2xl relative z-10">
        ...
        <div className="p-10 bg-slate-50 flex justify-between items-center border-t border-slate-100">
           <button onClick={prevStep} disabled={step === 1} className="px-10 py-5 text-slate-400 font-black text-[10px] uppercase tracking-widest hover:text-slate-900 transition-all flex items-center gap-3 disabled:opacity-0"><ArrowLeft size={16}/> Anterior</button>
           <button 
...
const MacroSlider = ({ label, value, onChange, min, max, step }: any) => (
  <div className="p-6 bg-slate-50 border border-slate-200 rounded-[2.5rem] space-y-4">
     ...
     <input type="range" min={min} max={max} step={step} value={value} onChange={e => onChange(Number(e.target.value))} className="w-full h-1.5 bg-slate-200 rounded-lg appearance-none accent-orange-600" />
  </div>
);
