...
const ChampionshipWizard: React.FC<{ onComplete: () => void, isTrial?: boolean }> = ({ onComplete, isTrial = false }) => {
  ...
  return (
    <div className="wizard-shell h-[75vh]">
      ...
      <header className="px-10 py-6 border-b border-white/5 bg-slate-900/50 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-5">
           ...
           <div>
              ...
              <p className="text-[8px] font-black uppercase text-orange-500 tracking-[0.4em] mt-1 opacity-70">Build v15.0 Protocol Standard</p>
           </div>
        </div>
        <div className="flex gap-1.5">
           {[1, 2, 3, 4, 5, 6].map((s) => (
             <div key={s} className={`w-8 h-1 rounded-full transition-all duration-700 ${step === s ? 'bg-orange-600 shadow-[0_0_15px_#f97316]' : step > s ? 'bg-orange-600/30' : 'bg-white/5'}`} />
           ))}
        </div>
      </header>
        ...
        <AnimatePresence mode="wait">
          {step === 1 && (
            <motion.div key="s1" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-10 max-w-5xl mx-auto">
               <WizardStepTitle icon={<LayoutGrid size={24}/>} title="Arquitetura de Arena" desc="Blueprint operacional base do seu império." />
                  ...
                  {CHAMPIONSHIP_TEMPLATES.map((tpl) => (
                    <button key={tpl.id} onClick={() => setSelectedTemplate(tpl)} className={`p-8 rounded-[2.5rem] border-2 transition-all text-left flex flex-col justify-between min-h-[220px] ${selectedTemplate?.id === tpl.id ? 'bg-orange-600 border-white shadow-2xl scale-[1.02]' : 'bg-slate-900/60 border-white/5 hover:border-orange-500/40'}`}>
                       ...
                       <div className="mt-6 flex items-center justify-between">
                          <span className={`px-4 py-1.5 rounded-xl text-[8px] font-black uppercase tracking-widest ${selectedTemplate?.id === tpl.id ? 'bg-white text-orange-600' : 'bg-white/5 text-slate-500'}`}>Node Sync v15</span>
          ...
          {step === 2 && (
            <motion.div key="s2" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-10 max-w-4xl mx-auto">
               <WizardStepTitle icon={<Globe size={24}/>} title="Baseline Monetária" desc="Configuração de capital e limites de arena." />
               <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                  <div className="bg-white/5 p-8 rounded-[3rem] border border-white/5 space-y-6">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                       <WizardSelect label="Moeda" val={formData.currency} onChange={v => setFormData({...formData, currency: v as CurrencyType})} options={[{v:'BRL',l:'REAL'},{v:'USD',l:'DÓLAR'}]} />
                       <WizardField label="Regiões" type="number" val={formData.regions_count} onChange={v => setFormData({...formData, regions_count: Number(v)})} />
                    </div>
                  </div>
                  <div className="bg-slate-900 p-8 rounded-[3rem] border border-white/10 flex flex-col justify-center space-y-6">
                    <div className="flex items-center gap-4 text-orange-500">
                       <Hourglass size={24} className="animate-pulse" />
                       <h4 className="font-black text-sm uppercase tracking-widest italic">Round Timer</h4>
                    </div>
                    <div className="grid grid-cols-2 gap-4">
          ...
          {step === 4 && (
            <motion.div key="s4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8">
               <WizardStepTitle icon={<Calculator size={24}/>} title="Auditória P00" desc="Estrutura de capital conforme CPC 26." />
          ...
          {(step === 3 || step === 5 || step === 6) && (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-8 max-w-4xl mx-auto text-center py-10">
               <WizardStepTitle icon={<Scale size={24}/>} title="Protocolo de Governança" desc="Revisão final antes da implantação live." />
               <div className="bg-slate-900/60 p-12 rounded-[4rem] border border-white/5 space-y-8 shadow-2xl">
                  ...
                  <h3 className="text-3xl font-black text-white uppercase italic">SincronizaçãoBaseline v15</h3>
                  <p className="text-lg text-slate-400 font-medium italic">"Todos os nodos configurados. Pronto para o despacho para a rede global."</p>
               </div>
            </motion.div>
          )}
        </AnimatePresence>
      ...
      <footer className="wizard-footer-dock">
         ...
         <div className="flex items-center gap-8">
            ...
            <button 
              ...
              className="px-16 py-6 bg-orange-600 text-white rounded-full font-black text-[11px] uppercase tracking-[0.4em] shadow-[0_20px_50px_rgba(249,115,22,0.4)] hover:bg-white hover:text-orange-950 transition-all flex items-center gap-6 group"
            >
               {isSubmitting ? (
...
const WizardStepTitle = ({ icon, title, desc }: any) => (
  <div className="flex items-center gap-6 border-b border-white/5 pb-8">
     <div className="p-4 bg-white/5 rounded-2xl text-orange-500 shadow-inner flex items-center justify-center">{icon}</div>
     <div>
        <h3 className="text-3xl font-black text-white uppercase italic tracking-tighter leading-none">{title}</h3>
        <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mt-1.5">{desc}</p>
     </div>
  </div>
);
...
const WizardField = ({ label, val, onChange, type = 'text', placeholder }: any) => (
  <div className="space-y-2">
     ...
     <input type={type} value={val} onChange={e => onChange(e.target.value)} className="w-full bg-slate-950 border border-white/10 rounded-2xl px-6 py-4 text-sm font-bold text-white outline-none focus:border-orange-500 transition-all shadow-inner" placeholder={placeholder} />
  </div>
);
