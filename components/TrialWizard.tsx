...
const TrialWizard: React.FC<{ onComplete: () => void }> = ({ onComplete }) => {
  ...
  const removeRegion = (idx: number) => {
    if (regionConfigs.length <= 1) return;
    const next = regionConfigs.filter((_, i) => i !== idx);
    setRegionConfigs(next);
    setFormData(prev => ({ ...prev, regionsCount: next.length }));
  };
  ...
  const addRegion = () => {
    ...
    setFormData(prev => ({ ...prev, regionsCount: regionConfigs.length + 1 }));
  };
    ...
    <div className="wizard-shell">
      ...
      <header className="wizard-header-fixed px-12 py-10 flex items-center justify-between border-b border-orange-500/30">
        ...
        <div className="flex items-center gap-10">
           <a href="https://ai.google.dev/gemini-api/docs/billing" target="_blank" className="px-8 py-3 bg-indigo-600 text-white rounded-full font-black text-[10px] uppercase tracking-widest hover:bg-white hover:text-indigo-950 transition-all shadow-xl shadow-indigo-600/20 flex items-center gap-3 active:scale-95">
                           ...
                           <div key={i} className="p-10 bg-slate-900 border-2 border-white/5 rounded-[4rem] group hover:border-orange-500/30 transition-all relative overflow-hidden shadow-2xl">
                              ...
                              <div className="space-y-10">
                                 <div className="space-y-3">
                                    ...
                                    <input value={r.name} onChange={e => updateRegion(i, { name: e.target.value })} className="w-full bg-slate-950 border border-white/10 rounded-2xl p-6 text-white font-black uppercase italic outline-none focus:border-orange-500 shadow-inner" />
                                 </div>

                                 <div className="grid grid-cols-2 gap-8">
                                    <div className="space-y-3">
                                       <label className="text-[9px] font-black uppercase text-slate-500 tracking-widest italic ml-2">Moeda Transacional</label>
                                       <select value={r.currency} onChange={e => updateRegion(i, { currency: e.target.value as CurrencyType })} className="w-full bg-slate-950 border border-white/10 rounded-2xl p-5 text-[10px] font-black text-white uppercase outline-none focus:border-orange-500 appearance-none cursor-pointer">
                                    ...
                                    <div className="space-y-3">
                                       <label className="text-[9px] font-black uppercase text-slate-500 tracking-widest italic ml-2">Demanda Alocada (%)</label>
                                       <input type="number" value={r.demand_weight} onChange={e => updateRegion(i, { demand_weight: parseInt(e.target.value) || 0 })} className="w-full bg-slate-950 border border-white/10 rounded-2xl p-5 text-center text-xl font-mono font-black text-orange-500 outline-none" />
                                    </div>
                                 </div>
                              </div>
                           </div>
              ...
              <motion.div key="s4" initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} className="space-y-12 pb-24">
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    <div className="space-y-6 bg-slate-900/60 p-8 rounded-[3rem] border border-white/5 shadow-2xl">
                       ...
                       <WizardField label="MP-B Base ($)" type="number" val={marketIndicators.prices.mp_b} onChange={(v:any)=>setMarketIndicators({...marketIndicators, prices: {...marketIndicators.prices, mp_b: parseFloat(v)}})} />
...
const WizardField = ({ label, val, onChange, type = 'text', placeholder, isLocked }: any) => (
  <div className="space-y-3 text-left group">
     ...
     <input type={type} value={val} onChange={e => !isLocked && onChange(e.target.value)} readOnly={isLocked} className={`w-full bg-slate-950 border-4 border-white/5 rounded-3xl px-8 py-5 text-lg font-bold text-white outline-none transition-all shadow-[inset_0_5px_15px_rgba(0,0,0,0.4)] placeholder:text-slate-800 font-mono ${isLocked ? 'cursor-not-allowed border-indigo-500/20 opacity-60' : 'focus:border-orange-600'}`} placeholder={placeholder} />
  </div>
);
