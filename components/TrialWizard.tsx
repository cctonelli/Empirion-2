...
const TrialWizard: React.FC<{ onComplete: () => void }> = ({ onComplete }) => {
  ...
  const fiduciaryMetrics = useMemo(() => {
    ...
    const installationsVal = isZeroMode ? 0 : (
    ...
    const property_deprec_rate_local = (tutorConfig.property_depreciation_rate !== undefined ? tutorConfig.property_depreciation_rate : 4) / 100;

    const building_deprec_only = buildingMode === 'owned' ? (buildingBaseValue * property_deprec_rate_local) : 0;
              ...
              <motion.div key="s3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-12 pb-20">
                 ...
                 <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 text-left animate-in fade-in slide-in-from-bottom-5 duration-300">
                    ...
                    <div className="bg-slate-900 border border-white/5 rounded-[3.5rem] p-10 space-y-8 hover:border-orange-500/30 transition-all shadow-2xl relative overflow-hidden">
                      ...
                      <WizardField label="INVESTIMENTO INSTALAÇÃO ($)" type="currency" currency={tutorConfig.currency} isLocked={tutorConfig.starting_mode === 'start_from_zero'} val={tutorConfig.machines[0].installation_cost ?? 150000.00} onChange={(v:any)=>{
                        const mac = [...tutorConfig.machines];
                    ...
                    <div className="bg-slate-900 border border-white/5 rounded-[3.5rem] p-10 space-y-8 hover:border-orange-500/30 transition-all shadow-2xl relative overflow-hidden">
                      ...
                      <WizardField label="INVESTIMENTO INSTALAÇÃO ($)" type="currency" currency={tutorConfig.currency} isLocked={tutorConfig.starting_mode === 'start_from_zero'} val={tutorConfig.machines[1].installation_cost ?? 600000.00} onChange={(v:any)=>{
                        const mac = [...tutorConfig.machines];
                    ...
                    <div className="bg-slate-900 border border-white/5 rounded-[3.5rem] p-10 space-y-8 hover:border-orange-500/30 transition-all shadow-2xl relative overflow-hidden">
                      ...
                      <WizardField label="INVESTIMENTO INSTALAÇÃO ($)" type="currency" currency={tutorConfig.currency} isLocked={tutorConfig.starting_mode === 'start_from_zero'} val={tutorConfig.machines[2].installation_cost ?? 1500000.00} onChange={(v:any)=>{
                        const mac = [...tutorConfig.machines];
              ...
              <motion.div key="s5" initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.98 }} className="space-y-12 pb-24">
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-4 gap-8 text-left animate-in fade-in zoom-in-95 duration-200">
                    ...
                    <div className="space-y-6 bg-slate-900/60 p-8 rounded-[3rem] border border-white/5 shadow-2xl relative">
                       ...
                       <WizardSelect label="PAGAR DIVIDENDOS A CADA" val={tutorConfig.dividend_frequency} onChange={(v:any)=>setTutorConfig({...tutorConfig, dividend_frequency: parseInt(v)})} options={[{v:'1',l:'TODOS ROUNDS'},{v:'2',l:'A CADA 2 ROUNDS'},{v:'4',l:'A CADA 4 ROUNDS'}]} />
                       <WizardSelect label="INCORPORAR LUCRO/PREJUÍZO NO CAPITAL SOCIAL" val={tutorConfig.profit_incorporation_frequency ?? 2} onChange={(v:any)=>setTutorConfig({...tutorConfig, profit_incorporation_frequency: parseInt(v)})} options={[{v:'1',l:'TODOS ROUNDS'},{v:'2',l:'A CADA 2 ROUNDS'},{v:'4',l:'A CADA 4 ROUNDS'}]} />
                             ...
                             <>
                              ...
                              <>
                                 ...
                                 <WizardField 
                                    ...
                                    type="number" 
                                    val={tutorConfig.rent_allocation_administrative ?? 20} 
                              ...
                              <div className="flex items-center gap-4 text-left">
                                 ...
                                 <div>
                                    ...
                                    <span className="text-sm font-black text-white font-mono">
                                       ...
                                       {' '} + Adm: <span className="text-sky-400">{tutorConfig.rent_allocation_administrative ?? 20}%</span>
                              ...
                              <div className="text-xs text-slate-400 md:max-w-md text-left md:text-right">
                                 {((tutorConfig.rent_allocation_productive ?? 70) + 
                                   ...
                                   (tutorConfig.rent_allocation_sales ?? 10)) === 100 ? (
                                    ...
                                    <span className="text-rose-400 font-extrabold font-mono">⚠ ERRO DE CONFORMIDADE: O rateio total deve ser rigidamente igual a 100% para continuar (Falta/Sobra de {(100 - ((tutorConfig.rent_allocation_productive ?? 70) + (tutorConfig.rent_allocation_administrative ?? 20) + (tutorConfig.rent_allocation_sales ?? 10)))}%).</span>
                                 )}
                              </div>
                           </div>
                        )}
                  ...
                  <div className="space-y-4">
                     ...
                     <div className="space-y-3 text-xs font-mono">
                        ...
                        <div className="flex justify-between items-center border-t border-white/5 pt-2">
                          <span className="text-slate-500 font-sans">Depreciação Provisória (Total):</span>
                        ...
                        <div className="pl-4 flex justify-between items-center text-[11px] text-slate-500">
                           ...
                           <span>{formatCurrency(fiduciaryMetrics.m_deprec_round, tutorConfig.currency)}</span>
                        </div>
                        <div className="pl-4 flex justify-between items-center text-[11px] text-slate-500">
                           <span>└ Deprec. Instalações ({fiduciaryMetrics.buildings_depreciation_rate_pct}%):</span>
                           <span>{formatCurrency(fiduciaryMetrics.installations_deprec_only, tutorConfig.currency)}</span>
                        </div>
                        {tutorConfig.building_mode === 'owned' && (
                          <div className="pl-4 flex justify-between items-center text-[11px] text-slate-500">
                             <span>└ Deprec. Imóvel Próprio ({fiduciaryMetrics.property_depreciation_rate_pct}%):</span>
                             <span>{formatCurrency(fiduciaryMetrics.building_deprec_only, tutorConfig.currency)}</span>
                          </div>
                        )}
                        ...
                        <div className="flex flex-col border-t border-white/5 pt-2">
                          ...
                          {tutorConfig.building_mode === 'rented' && (
                            <div className="mt-2 pl-4 border-l border-orange-500/20 space-y-1.5 text-[11px] text-slate-400">
                              ...
                              <div className="flex justify-between">
                                <span className="text-slate-500">└── Aluguel OPEX (Adm+Sales) ({((tutorConfig.rent_allocation_administrative ?? 10) + (tutorConfig.rent_allocation_sales ?? 10))}%):</span>
                                <span className="text-sky-400 font-bold">{formatCurrency(((tutorConfig.monthly_rent_value ?? 50000.00) * ((tutorConfig.rent_allocation_administrative ?? 20) + (tutorConfig.rent_allocation_sales ?? 10))) / 100, tutorConfig.currency)}</span>
                              </div>
                            </div>
                          )}
                        </div>
