    ...
    <div className="space-y-12 animate-in fade-in duration-700 pb-20 px-4">
      ...
      <div className="flex flex-row overflow-x-auto gap-8 pb-8 pt-2 px-2 scroll-smooth custom-scrollbar snap-x snap-mandatory">
         {filteredChamps.map((champ) => (
            <div key={champ.id} className={`bg-slate-900/50 backdrop-blur-xl p-5 md:p-6 rounded-[2rem] border transition-all duration-500 relative overflow-hidden shadow-2xl flex-shrink-0 w-[85vw] md:w-[460px] snap-start ${expandedArena === champ.id ? 'border-orange-500/50 ring-4 ring-orange-500/10' : 'border-white/5'}`}>
               ...
               <div className="space-y-4">
                 ...
                 <div className="space-y-1">
                    <h3 className="text-2xl font-black text-white uppercase italic tracking-tight leading-none">{champ.name}</h3>
                    <div className="flex items-center gap-4 pt-1">
                       <div className="flex items-center gap-2 text-[10px] font-bold text-slate-500 uppercase">
                          <Users size={12} className="text-orange-500" /> {champ.teams?.length || 0} Equipes
                       </div>
                       <div className="flex items-center gap-2 text-[10px] font-bold text-slate-500 uppercase">
                          <Terminal size={12} className="text-blue-500" /> Ciclo 0{champ.current_round}
                       </div>
                    </div>
                 </div>
                 ...
                 {expandedArena !== champ.id ? (
                   <button 
                     ...
                     className="w-full py-3 bg-orange-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-white hover:text-orange-600 transition-all shadow-xl flex items-center justify-center gap-3 active:scale-95"
                   >
                     Ver Equipes & Bots <ChevronRight size={14} />
                   </button>
                 ) : (
                   <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-2 pt-3 border-t border-white/5">
                         ...
                         {champ.teams && champ.teams.length > 0 ? (
                           champ.teams.map((team: Team) => (
                             <button 
                               ...
                               className={`w-full p-2.5 bg-slate-950/80 border rounded-xl flex items-center justify-between group transition-all text-left ${
                                 team.is_bot 
                                ...
                                <div className="flex items-center gap-3">
                                   <div className={`w-8 h-8 rounded-lg flex items-center justify-center transition-colors ${
                                     team.is_bot ? 'bg-indigo-600/20 text-indigo-400' : 'bg-white/5 text-orange-500 group-hover:bg-white group-hover:text-orange-600'
                                      ...
                                      {team.is_bot ? <Bot size={16}/> : <Shield size={14} />}
                                   </div>
                                   <div className="flex flex-col">
                                      <span className="text-xs font-black text-white uppercase italic leading-tight">{team.name}</span>
                                      {team.is_bot && <span className="text-[7px] font-black text-indigo-400 uppercase tracking-widest mt-0.5 leading-none">Autonomous Synthetic Node</span>}
                                   </div>
                                </div>
                                <div className="flex items-center gap-2">
                                   {team.is_bot ? (
                                     <div className="px-2 py-0.5 bg-indigo-600/20 border border-indigo-500/30 rounded-md flex items-center gap-1.5">
                                        <div className="w-1 h-1 bg-indigo-400 rounded-full animate-pulse" />
                                        <span className="text-[7px] font-black text-indigo-400 uppercase">AI OPERATING</span>
                                     </div>
                                   ) : (
                                     <div className="flex items-center gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <span className="text-[8px] font-black text-white uppercase tracking-wider">Comandar</span>
                           ...
                           <div className="py-6 text-center space-y-2">
                             <p className="text-[9px] text-slate-500 italic uppercase font-black">Nenhuma equipe detectada.</p>
