...
const BusinessPlanWizard: React.FC<IntegratedWizardProps> = ({ championshipId, teamId, currentRound = 1, onClose, isReadOnly = false }) => {
  ...
  const handleAISuggest = async (stepIdx: number) => {
    ...
    const suggestion = await generateBusinessPlanField(
      ...
      `Com base no framework de Osterwalder, gere uma análise para ${BP_STEPS[stepIdx].label}. Considere o histórico R-0 (INICIAL) - R-${currentRound-1}: ${JSON.stringify(simHistory.map(h => ({ round: h.round, kpis: h.kpis })))}. Analise a coesão entre os blocos do canvas.`, 
      branch
    );
