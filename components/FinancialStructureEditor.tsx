...
const FinancialStructureEditor: React.FC<FinancialStructureEditorProps> = ({ onChange, initialBalance = [], initialDRE = [], initialCashFlow = [] }) => {
  ...
  const isBalanced = Math.abs(assets - liabPL) < 0.05; 
    ...
    <div className="space-y-10">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 bg-slate-900/60 p-8 rounded-[3rem] border border-white/5 shadow-2xl">
        <div className="flex flex-wrap gap-2 p-1.5 bg-slate-950 rounded-[1.75rem] w-fit border border-white/5 shadow-inner">
          <button onClick={() => setActiveTab('balance')} className={`px-8 py-3 rounded-2xl text-[9px] font-black uppercase tracking-widest transition-all flex items-center gap-2 ${activeTab === 'balance' ? 'bg-orange-600 text-white shadow-lg' : 'text-slate-500 hover:text-slate-300'}`}><Landmark size={14} /> Balanço Patrimonial</button>
          <button onClick={() => setActiveTab('dre')} className={`px-8 py-3 rounded-2xl text-[9px] font-black uppercase tracking-widest transition-all flex items-center gap-2 ${activeTab === 'dre' ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-500 hover:text-slate-300'}`}><TrendingUp size={14} /> DRE Tático</button>
          <button onClick={() => setActiveTab('cashflow')} className={`px-8 py-3 rounded-2xl text-[9px] font-black uppercase tracking-widest transition-all flex items-center gap-2 ${activeTab === 'cashflow' ? 'bg-emerald-600 text-white shadow-lg' : 'text-slate-500 hover:text-slate-300'}`}><Activity size={14} /> Fluxo de Caixa</button>
        </div>
        ...
        <div className="flex flex-wrap gap-4 items-center">
           ...
           <div className={`px-6 py-4 rounded-2xl border flex items-center gap-3 transition-all duration-700 ${isBalanced ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400' : 'bg-rose-500/10 border-rose-500/30 text-rose-400 animate-pulse'}`}>
              ...
              <span className="text-[9px] font-black uppercase italic tracking-[0.2em]">
                {isBalanced ? 'Audit Balanced' : `Divergência: $ ${formatMoney(assets - liabPL)}`}
              </span>
           </div>
        </div>
      </div>
      ...
      <div className="matrix-container p-6 md:p-10">
        <div className="space-y-4 min-w-[900px]">
          {Array.isArray(nodes) && nodes.length > 0 ? nodes.map((node) => (
            <TreeNode key={node.id} node={node} onUpdate={updateNode} onAdd={addSubNode} onRemove={removeNode} tabType={activeTab} />
          )) : (
            <div className="py-32 text-center bg-slate-900/40 rounded-[3.5rem] border border-dashed border-white/5 text-slate-500 uppercase font-black text-[10px] tracking-[0.5em]">Sincronizando Módulos Oracle Master...</div>
          )}
        </div>
      </div>
    </div>
  ...
  return (
    <div className="space-y-3">
      <div className={`group flex items-center gap-5 p-4 rounded-2xl transition-all border ${isParent ? 'bg-slate-900/80 border-white/10 shadow-lg' : 'bg-white/5 border-white/5 hover:border-white/20'}`} style={{ marginLeft: level * 32 }}>
        <button onClick={() => setIsOpen(!isOpen)} className={`p-2 text-slate-600 hover:text-white transition-all ${!isParent && 'opacity-0 pointer-events-none'} ${isOpen ? 'rotate-0' : '-rotate-90'}`}><ChevronDown size={18}/></button>
        
        <div className="flex-1 flex items-center justify-between gap-10">
           <div className="flex-1 flex items-center gap-3">
              <input readOnly={node.isReadOnly || !node.isEditable} className={`bg-transparent outline-none font-black text-sm flex-1 transition-colors ${isParent ? (tabType === 'dre' ? 'text-blue-400' : tabType === 'cashflow' ? 'text-emerald-400' : 'text-orange-500') + ' uppercase italic tracking-tight' : 'text-slate-300'}`} value={node.label} onChange={e => onUpdate(node.id, { label: e.target.value })} />
           ...
           <div className={`flex items-center gap-4 bg-slate-950 px-6 py-2.5 rounded-xl border border-white/5 shadow-inner transition-all group-hover:border-orange-500/30`}>
             ...
             {(isParent || node.isReadOnly) ? (
               <span className={`font-mono font-black text-sm ${isNegative ? 'text-rose-400' : 'text-white'} italic`}>{formatMoney(node.value)}</span>
             ) : (
               <input 
                 type="text" 
                 className={`w-36 bg-transparent outline-none font-mono font-bold text-sm ${isNegative ? 'text-rose-400' : 'text-white'}`} 
                 value={formatMoney(node.value)} 
                 onChange={e => {
                   const raw = e.target.value.replace(/[^-0-9,.]/g, '').replace(',', '.');
                   onUpdate(node.id, { value: parseFloat(raw) || 0 });
                 }} 
               />
             )}
           </div>
           ...
           <div className="flex gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
              {canAdd && <button onClick={() => onAdd(node.id)} className="p-2.5 text-blue-400 hover:bg-blue-600 hover:text-white rounded-xl transition-all shadow-lg active:scale-90"><Plus size={16}/></button>}
              {canDelete && <button onClick={() => onRemove(node.id)} className="p-2.5 text-rose-500 hover:bg-rose-600 hover:text-white rounded-xl transition-all shadow-lg active:scale-90"><Trash2 size={16}/></button>}
           </div>
        </div>
      </div>
      {isOpen && isParent && <div className="space-y-3 relative"><div className="absolute left-[24px] top-0 bottom-0 w-px bg-white/5" />{node.children!.map(child => <TreeNode key={child.id} node={child} onUpdate={onUpdate} onAdd={onAdd} onRemove={onRemove} level={level + 1} tabType={tabType} />)}</div>}
    </div>
  );
};
...
const StatusBox = ({ label, val, color }: any) => (
  <div className={`p-4 px-10 rounded-[2rem] border shadow-2xl transition-all hover:scale-105 ${color === 'orange' ? 'bg-orange-600 text-white border-orange-500 shadow-orange-600/10' : 'bg-blue-600 text-white border-blue-500 shadow-blue-600/10'}`}>
     ...
     <span className="text-2xl font-black font-mono italic leading-none">$ {formatMoney(val)}</span>
  </div>
);
