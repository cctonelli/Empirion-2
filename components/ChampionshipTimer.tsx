...
import { Clock, AlertCircle, Hourglass, Pause, Trophy } from 'lucide-react';
...
const ChampionshipTimer: React.FC<ChampionshipTimerProps> = ({ 
      ...
      <div 
        className={`px-4 py-1.5 rounded-xl border flex items-center gap-4 transition-all duration-500 shadow-lg ${
          isTournamentFinished
            ? 'bg-emerald-950/80 border-emerald-500/50 text-emerald-200 shadow-[0_0_15px_rgba(16,185,129,0.15)] animate-in fade-in'
            : isPaused 
      ...
      >
        <div className="flex flex-col min-w-[80px]">
          <span className={`text-[7px] font-black uppercase tracking-[0.2em] mb-0.5 leading-none ${
            isTournamentFinished ? 'text-emerald-400' : isPaused ? 'text-amber-400' : isUrgent || isCritical ? 'text-white/90' : 'text-orange-400'
          }`}>
            {isTournamentFinished ? 'STATUS ARENA' : isPaused ? 'CONGELADO' : isUrgent ? 'URGENTE' : isCritical ? 'PRAZO FINAL' : 'RESTA'}
          </span>
        ...
        <div className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all shadow-md flex-shrink-0 ${
          isTournamentFinished ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/30' :
          ...
          {isTournamentFinished ? <Trophy size={18} className="text-emerald-400 animate-bounce" /> : isPaused ? <Pause size={20} strokeWidth={2.5} /> : isUrgent ? <AlertCircle size={22} strokeWidth={2.5} /> : <Clock size={22} strokeWidth={2} className={!isCritical ? 'animate-pulse' : ''} />}
        </div>
      </div>
    );
  }
  ...
  return (
    <div className={`px-8 py-4 rounded-[2rem] shadow-2xl border transition-all duration-700 flex items-center gap-6 ${
      isTournamentFinished ? 'bg-emerald-950/85 border-emerald-600/30 text-emerald-200' :
      ...
      <div className="flex flex-col">
        <span className={`text-[8px] font-black uppercase tracking-[0.2em] mb-1 ${
          isTournamentFinished ? 'text-emerald-400' : isPaused ? 'text-amber-400' : isUrgent || isCritical ? 'text-white' : 'text-orange-500'
        }`}>
          {isTournamentFinished ? 'ARENA CONCLUÍDA' : isPaused ? 'DECISÕES CONGELADAS' : isUrgent ? 'URGENTE: TRANSMITA' : isCritical ? 'PRAZO FINAL' : 'RESTA PARA DECIDIR'}
        </span>
      ...
      <div className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-all shadow-lg ${
        isTournamentFinished ? 'bg-emerald-600/25 text-emerald-400 border border-emerald-500/20' :
        ...
        {isTournamentFinished ? <Trophy size={28} className="text-emerald-400 animate-bounce" /> : isPaused ? <Pause size={28} /> : isUrgent ? <AlertCircle size={32} /> : <Clock size={32} className={!isCritical ? 'animate-pulse' : ''} />}
      </div>
    </div>
  );
};
