...
import { 
  ...
  UserPlus, UserMinus, ShieldQuestion, Globe2
} from 'lucide-react';
import { 
  getChampionships, 
  deleteChampionship, 
...
const AdminCommandCenter: React.FC<{ preTab?: string }> = ({ preTab = 'system' }) => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (session) {
        ...
        setProfile(prof);
        // Se for Tutor, a aba inicial deve ser Arenas
      ...
      // Simulação de carregamento de menus das constantes
  ...
  // Visão do Tutor: Controle de Arena Específica
  if (selectedArena) {
    return (
        ...
        <header className="flex flex-col md:flex-row items-center justify-between px-6 gap-6">
           <div className="flex items-center gap-6">
              ...
              <div>
                 ...
                 <p className="text-slate-500 text-[11px] font-black uppercase tracking-[0.4em] mt-3 italic">Ambiente do Tutor • Orquestração P0{selectedArena.current_round}</p>
              </div>
           </div>
        </header>
  ...
  return (
    <div className="space-y-12 animate-in fade-in duration-700 pb-20 font-sans max-w-[1600px] mx-auto">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 px-6 pt-6">
        <div className="space-y-2">
          ...
          <p className="text-slate-500 font-black text-[12px] uppercase tracking-[0.4em] italic pl-2">
            {isAdmin ? 'Governança Global da Plataforma • Dono da Bola' : 'Gestão de Arenas Atribuídas'}
          </p>
        </div>
        ...
        <div className="flex items-center gap-6 bg-slate-900 border border-white/5 rounded-[2.5rem] p-6 shadow-2xl">
           <div className="flex items-center gap-4">
              ...
              <div className="flex flex-col">
                 ...
                 <span className="text-[9px] font-black text-slate-500 uppercase tracking-[0.3em]">{isAdmin ? 'PROPRIETÁRIO DO SISTEMA' : 'TUTOR CERTIFICADO'}</span>
              </div>
           </div>
        </div>
      </div>

      {/* Navegação de Tabs Exclusiva por Cargo */}
      <div className="flex gap-4 px-6 overflow-x-auto no-scrollbar pb-4 sticky top-0 z-[100] bg-[#020617]/80 backdrop-blur-xl py-4 border-b border-white/5">
         {isAdmin && <NavTab active={activeTab === 'system'} onClick={() => setActiveTab('system')} label="Saúde do Sistema" color="indigo" />}
         <NavTab active={activeTab === 'tournaments'} onClick={() => setActiveTab('tournaments')} label="Gestão de Arenas" color="orange" />
         {isAdmin && (
           <>
             ...
             <NavTab active={activeTab === 'templates'} onClick={() => setActiveTab('templates')} label="Blueprints Oracle" color="emerald" />
           </>
         )}
      </div>
        ...
        <AnimatePresence mode="wait">
          {activeTab === 'system' && isAdmin && (
            <motion.div key="sys" initial={{opacity:0}} animate={{opacity:1}} className="space-y-12">
               <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
                  ...
                  <MetricCard label="Receita SaaS (Mês)" val="R$ 48.900" icon={<CreditCard className="text-amber-500"/>} trend="+5.4%" />
                  <MetricCard label="Latência Média" val="14ms" icon={<Cpu className="text-indigo-500"/>} trend="Optimal" />
               </div>
               <div className="bg-slate-900 p-12 rounded-[4rem] border border-white/5 shadow-2xl">
                  <h3 className="text-2xl font-black text-white uppercase italic mb-8 flex items-center gap-4"><Gauge className="text-indigo-400" /> Monitoramento de Nodos</h3>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                     <StatusNode label="Nodo Brasil" ping="8ms" status="healthy" />
                     <StatusNode label="Nodo USA" ping="84ms" status="healthy" />
            ...
            !showWizard ? (
              <motion.div key="list" initial={{opacity:0}} animate={{opacity:1}} className="space-y-16">
                 <div className="flex flex-col md:flex-row justify-between items-center gap-10 bg-slate-900 p-16 rounded-[5rem] border border-white/5 shadow-2xl relative overflow-hidden group">
                    <div className="flex items-center gap-10 relative z-10">
                       ...
                       <div>
                          <h3 className="text-5xl font-black text-white uppercase italic tracking-tight leading-none">Orquestrar Arena</h3>
                    ...
                    <button onClick={() => setShowWizard(true)} className="px-20 py-8 bg-white text-slate-950 rounded-full font-black text-sm uppercase tracking-[0.4em] hover:bg-orange-600 hover:text-white transition-all shadow-2xl active:scale-95 z-10 shrink-0">Iniciar Protocolo</button>
                 </div>
                         ...
                         <div>
                            <div className="flex justify-between items-start mb-12">
                               <span className={`px-6 py-2 rounded-full text-[10px] font-black uppercase tracking-[0.3em] ${champ.is_trial ? 'bg-orange-600/10 text-orange-500' : 'bg-emerald-500/10 text-emerald-500'}`}>
                                 {champ.is_trial ? 'Arena Trial' : 'Arena Live'}
                               </span>
                            ...
                            <p className="text-[11px] font-black text-slate-500 uppercase tracking-widest italic">{champ.branch} Core Node</p>
                         </div>
              ...
              <motion.div key="wizard" initial={{opacity:0, scale:0.98}} animate={{opacity:1, scale:1}}>
                <div className="flex justify-between items-center mb-8 px-4">
                  <h2 className="text-3xl font-black text-white uppercase italic">Implantar Novo Nodo</h2>
                  <button onClick={() => setShowWizard(false)} className="p-6 bg-rose-600/10 hover:bg-rose-600 text-rose-500 hover:text-white rounded-2xl flex items-center gap-3 font-black text-xs uppercase tracking-[0.2em] transition-all"><X size={20} /> Abortar</button>
                </div>
          ...
          {activeTab === 'uidesign' && isAdmin && (
            <motion.div key="cms" initial={{opacity:0, x:20}} animate={{opacity:1, x:0}} className="grid grid-cols-1 lg:grid-cols-2 gap-12">
               {/* CMS NAVEGAÇÃO E MENUS */}
               <div className="bg-slate-900 p-12 rounded-[4rem] border border-white/5 space-y-10 shadow-2xl">
                  <div className="space-y-2">
                     <h3 className="text-3xl font-black text-white uppercase italic flex items-center gap-4"><MenuIcon className="text-indigo-400" /> Gestão de Navegação</h3>
                     <p className="text-xs text-slate-500 font-bold uppercase tracking-widest italic">Edite a estrutura de menus e links do site.</p>
                  </div>
                  <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                     {menus.map((m) => (
                       <div key={m.id} className="p-6 bg-white/5 border border-white/10 rounded-2xl flex items-center justify-between group">
                          <div className="flex items-center gap-4">
                             ...
                             <input value={m.label} onChange={(e) => {}} className="bg-transparent border-none text-white font-black uppercase text-sm outline-none" />
                          </div>
                     ...
                     <button className="w-full py-4 border-2 border-dashed border-white/10 rounded-2xl text-slate-500 font-black uppercase text-[10px] hover:text-white hover:border-white/20 transition-all flex items-center justify-center gap-3">
                        <Plus size={16} /> Adicionar Item de Menu
                     </button>
                  </div>
                  <button className="w-full py-8 bg-indigo-600 text-white rounded-[2rem] font-black text-sm uppercase tracking-[0.4em] flex items-center justify-center gap-4 shadow-2xl hover:bg-white hover:text-indigo-950 transition-all">
                     <Save size={24} /> Sincronizar Menus
                  </button>
               </div>
               ...
               <div className="bg-slate-900 p-12 rounded-[4rem] border border-white/5 space-y-10 shadow-2xl">
                  <div className="space-y-2">
                     ...
                     <p className="text-xs text-slate-500 font-bold uppercase tracking-widest italic">Controle visual e conteúdo do Hero Principal.</p>
                  </div>
                  <div className="space-y-8">
                     <div className="space-y-3">
                        <label className="text-[10px] font-black text-slate-600 uppercase tracking-widest">Título do Hero</label>
                     ...
                     <div className="grid grid-cols-2 gap-8">
                        ...
                        <div className="space-y-3 text-center">
                           <label className="text-[10px] font-black text-slate-600 uppercase tracking-widest">Oracle Node</label>
                           <div className="p-4 bg-emerald-500/10 rounded-2xl text-emerald-500 font-black text-xs">{PROTOCOL_NODE}</div>
                        </div>
                     </div>
                  </div>
                  <button className="w-full py-8 bg-orange-600 text-white rounded-[2rem] font-black text-sm uppercase tracking-[0.4em] flex items-center justify-center gap-4 shadow-2xl hover:bg-white hover:text-orange-950 transition-all">
                     <Save size={24} /> Publicar Branding
                  </button>
               </div>
            </motion.div>
          )}
          ...
          {activeTab === 'users' && isAdmin && (
            <motion.div key="users" initial={{opacity:0, scale:0.98}} animate={{opacity:1, scale:1}} className="bg-slate-900 p-12 rounded-[4rem] border border-white/5 shadow-2xl space-y-10">
               <div className="flex justify-between items-center border-b border-white/5 pb-8">
                  <div className="space-y-2">
                     <h3 className="text-3xl font-black text-white uppercase italic flex items-center gap-4"><Users className="text-emerald-500" /> Governança de Usuários</h3>
                     <p className="text-xs text-slate-500 font-bold uppercase tracking-widest italic">Gerencie permissões e funções (Roles) no cluster.</p>
                  </div>
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                  {/* Mock de Usuários para Exemplo de UI */}
                  {['Tutor Master', 'Alpha Strategist', 'Observer Charlie'].map((name, i) => (
                    <div key={i} className="p-8 bg-white/5 border border-white/10 rounded-[3rem] space-y-6 hover:border-emerald-500/30 transition-all group">
                       ...
                       <div>
                          ...
                          <p className="text-[10px] font-bold text-slate-600 uppercase mt-1">auth-node-0{i+1}@empirion.ia</p>
                       </div>
          ...
          {activeTab === 'templates' && isAdmin && (
            <motion.div key="tpl" initial={{opacity:0, scale:0.95}} animate={{opacity:1, scale:1}} className="grid grid-cols-1 md:grid-cols-3 gap-12">
               ...
               <button className="border-4 border-dashed border-white/5 rounded-[4rem] flex flex-col items-center justify-center gap-6 p-12 text-slate-600 hover:text-white hover:border-white/20 transition-all group">
                  ...
                  <span className="text-xl font-black uppercase tracking-tighter italic">Novo Ramo / Blueprint</span>
               </button>
            </motion.div>
          )}
        </AnimatePresence>
