...
const AdminCommandCenter: React.FC<{ preTab?: string }> = ({ preTab = 'tournaments' }) => {
  ...
  // Componente de Dashboard do Site (Métricas de Negócio, não de Jogo)
  const SystemMetrics = () => (
    <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
       <MetricCard label="Usuários Online" val="1.284" icon={<Users className="text-emerald-500"/>} trend="+12%" />
       <MetricCard label="Arenas Ativas" val={championships.length.toString()} icon={<Globe className="text-blue-500"/>} trend="Stable" />
       <MetricCard label="Vendas SaaS (Mês)" val="R$ 42.900" icon={<CreditCard className="text-amber-500"/>} trend="+5.4%" />
       <MetricCard label="Uso de CPU IA" val="32%" icon={<Cpu className="text-indigo-500"/>} trend="Low Latency" />
    </div>
  );
      ...
      <div className="space-y-8 animate-in fade-in duration-500 pb-20 font-sans max-w-[1600px] mx-auto">
        <header className="flex flex-col md:flex-row items-center justify-between px-6 gap-6">
           <div className="flex items-center gap-6">
              ...
              <div>
                 <h1 className="text-4xl font-black text-white uppercase italic tracking-tighter leading-none">Control Room: <span className="text-orange-500">{selectedArena.name}</span></h1>
    ...
    <div className="space-y-12 animate-in fade-in duration-700 pb-20 font-sans max-w-[1600px] mx-auto">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 px-6 pt-6">
        <div className="space-y-2">
          ...
          <p className="text-slate-500 font-black text-[12px] uppercase tracking-[0.4em] italic pl-2">Governança Global da Plataforma • Dono da Bola</p>
        </div>
        
        <div className="flex items-center gap-6 bg-slate-900 border border-white/5 rounded-[2.5rem] p-6 shadow-2xl">
           <div className="flex items-center gap-4">
              <div className="w-3.5 h-3.5 bg-blue-500 rounded-full animate-pulse shadow-[0_0_15px_#3b82f6]" />
              <div className="flex flex-col">
                 <span className="text-lg font-black text-white uppercase italic tracking-tight">{profile?.name || 'ADMIN MASTER'}</span>
                 <span className="text-[9px] font-black text-slate-500 uppercase tracking-[0.3em]">PROPRIETÁRIO DO SISTEMA</span>
              </div>
           </div>
        </div>
      </div>
      ...
      <div className="flex gap-4 px-6 overflow-x-auto no-scrollbar pb-4 sticky top-0 z-[100] bg-[#020617]/80 backdrop-blur-xl py-4 border-b border-white/5">
         ...
         {isAdmin && (
           <>
             <NavTab active={activeTab === 'uidesign'} onClick={() => setActiveTab('uidesign')} label="Branding & CMS" color="indigo" />
             <NavTab active={activeTab === 'templates'} onClick={() => setActiveTab('templates')} label="Oracle Templates" color="emerald" />
             <NavTab active={activeTab === 'system'} onClick={() => setActiveTab('system')} label="Saúde do Sistema" color="indigo" />
           </>
         )}
      </div>
          ...
          {activeTab === 'system' && (
            <motion.div key="sys" initial={{opacity:0}} animate={{opacity:1}} className="space-y-12">
               ...
               <div className="bg-slate-900 p-12 rounded-[4rem] border border-white/5 shadow-2xl">
                  <h3 className="text-2xl font-black text-white uppercase italic mb-8 flex items-center gap-4"><Gauge className="text-indigo-400" /> Latência por Região</h3>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                     <StatusNode label="Nodo Brasil" ping="12ms" status="healthy" />
                     <StatusNode label="Nodo USA" ping="84ms" status="healthy" />
                     <StatusNode label="Nodo Europa" ping="142ms" status="warning" />
                     <StatusNode label="Nodo Asia" ping="210ms" status="healthy" />
                  </div>
               </div>
            </motion.div>
          )}
                      ...
                      <div key={champ.id} className="bg-slate-900/50 p-12 rounded-[4rem] border border-white/5 shadow-2xl hover:border-orange-500/40 transition-all flex flex-col justify-between min-h-[420px]">
                         <div>
                            <div className="flex justify-between items-start mb-12">
                               <span className="px-6 py-2 rounded-full text-[10px] font-black uppercase tracking-[0.3em] bg-emerald-500/10 text-emerald-500 border border-emerald-500/20">Arena Online</span>
            ...
            <motion.div key="cms" initial={{opacity:0, x:20}} animate={{opacity:1, x:0}} className="grid grid-cols-1 lg:grid-cols-2 gap-12">
               ...
               <div className="bg-slate-900 p-16 rounded-[4rem] border border-white/5 space-y-12 shadow-2xl">
                  <div className="space-y-2">
                     ...
                     <p className="text-xs text-slate-500 font-bold uppercase tracking-widest italic">Controle cores e escala visual do sistema.</p>
                  </div>
                  ...
                  <div className="p-6 bg-orange-600/5 border border-orange-500/20 rounded-[2rem] flex items-center gap-5">
                     ...
                     <p className="text-[10px] text-slate-400 font-black uppercase leading-tight tracking-widest italic">Alterações impactam o tempo de renderização em todos os nodos de equipes.</p>
                  </div>
               </div>
            </motion.div>
          )}
            ...
            <motion.div key="tpl" initial={{opacity:0, scale:0.95}} animate={{opacity:1, scale:1}} className="grid grid-cols-1 md:grid-cols-3 gap-12">
               {CHAMPIONSHIP_TEMPLATES.map(tpl => (
                 <div key={tpl.id} className="bg-slate-900 border border-white/5 p-12 rounded-[4rem] flex flex-col justify-between min-h-[480px] shadow-2xl group hover:border-emerald-500/40 transition-all">
                    ...
                    <button className="text-[10px] font-black uppercase text-blue-400 hover:text-white tracking-[0.3em] transition-all flex items-center gap-3">Edit Master Blueprint <ArrowRight size={16}/></button>
                 </div>
               ))}
               <button className="border-4 border-dashed border-white/5 rounded-[4rem] flex flex-col items-center justify-center gap-6 p-12 text-slate-600 hover:text-white hover:border-white/20 transition-all group">
                  ...
                  <span className="text-xl font-black uppercase tracking-tighter italic">Novo Template Master</span>
               </button>
            </motion.div>
          )}
        </AnimatePresence>
