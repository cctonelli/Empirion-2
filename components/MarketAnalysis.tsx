...
import { Sparkles, TrendingUp, AlertTriangle, Lightbulb, RefreshCw, Search, Globe, Link as LinkIcon, Loader2, ArrowRight, Landmark, ArrowUpRight, ArrowDownRight, Activity, ShieldAlert, Lock } from 'lucide-react';
    ...
    <div className="max-w-7xl mx-auto space-y-10 pb-32 animate-in fade-in duration-700 px-4">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-slate-200 pb-8">
        <div>
          ...
          <p className="text-slate-500 mt-2 font-medium text-lg">Predictive forecasting and real-time local processing.</p>
        </div>
        <button 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-10">
          ...
          <div className="relative z-10 h-full flex flex-col justify-between space-y-12">
            <div className="space-y-6">
              <div className="inline-flex items-center gap-3 px-5 py-2.5 bg-white/10 backdrop-blur-xl rounded-full text-[10px] font-black uppercase tracking-widest border border-white/10">
                <TrendingUp size={14} className="text-emerald-400" /> Local Strategic Forecast
              </div>
        ...
        <div className="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm space-y-8 flex flex-col">
           <div className="flex items-center gap-3 mb-2">
              ...
              <h3 className="text-lg font-black text-slate-900 uppercase tracking-tight">Market Pulse</h3>
           </div>
              ...
              {[8, 1, 5].map(id => (
                <div key={id} className="flex items-center justify-between group hover:bg-slate-50 p-2 rounded-xl transition-all">
                   <div className="flex flex-col">
                      <span className="text-[10px] font-black text-slate-400">UNIT 0{id}</span>
                      <span className="text-xs font-bold text-slate-900">SIND Ind.</span>
                   </div>
                   <div className="text-right">
                      <span className="block text-sm font-black font-mono">1.12</span>
                      <span className="text-[9px] font-black flex items-center gap-1 justify-end text-emerald-500">
        ...
        <div className="lg:col-span-2 bg-white p-12 rounded-[4rem] border border-slate-100 shadow-sm space-y-12 overflow-hidden relative">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
             <div className="flex items-center gap-4">
                <div className={`p-4 ${isTrial ? 'bg-slate-100 text-slate-400' : 'bg-slate-900 text-white'} rounded-3xl shadow-xl`}>
                   {isTrial ? <Lock size={28} /> : <Search size={28} />}
                </div>
                <div>
                   <h3 className={`text-2xl font-black uppercase tracking-tight ${isTrial ? 'text-slate-400' : 'text-slate-900'}`}>Grounded Intelligence Search</h3>
          ...
          {isTrial ? (
             <div className="bg-slate-50 p-12 rounded-[3rem] border-2 border-dashed border-slate-200 flex flex-col items-center text-center space-y-6">
                <ShieldAlert size={48} className="text-slate-300" />
                <div className="space-y-2">
                   <h4 className="text-xl font-black text-slate-400 uppercase italic">Ferramenta Externa Bloqueada</h4>
                   <p className="text-sm font-bold text-slate-500 uppercase tracking-widest">
                      O Grounding via Google Search é um recurso exclusivo das arenas <span className="text-orange-600">FIDELITY LIVE</span>. <br/>
                      No modo Trial, utilize apenas os dados processados diretamente pelo motor local.
                   </p>
                </div>
             </div>
          ) : (
            <form onSubmit={handleSearch} className="relative z-10 group">
              ...
              <input 
                ...
                placeholder="Search for industrial trends..."
