...
const Dashboard: React.FC = () => {
  const lineChartOptions: any = {
    ...
    xaxis: { categories: ['R0', 'R1', 'R2', 'R3', 'R4'], axisBorder: { show: false }, axisTicks: { show: false } },
  ...
  const radialOptions: any = {
    ...
    plotOptions: {
      radialBar: {
        ...
        dataLabels: {
          name: { show: false },
          value: { offsetY: 5, fontSize: '20px', fontWeight: 'bold', color: '#0f172a' }
        }
      }
    },
    colors: ['#3b82f6']
  ...
  return (
    <div className="space-y-10 animate-in fade-in slide-in-from-top-4 duration-700">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-slate-500 mt-1 flex items-center gap-2 font-medium">
            <Briefcase size={16} /> Alpha Solutions Group | Round 4
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          { label: 'Net Profit', value: 'R$ 32.5k', trend: '+12%', icon: DollarSign, color: 'text-blue-600', bg: 'bg-blue-50 shadow-blue-100' },
          { label: 'Market Share', value: '24.8%', trend: '+4%', icon: Target, color: 'text-emerald-600', bg: 'bg-emerald-50 shadow-emerald-100' },
          { label: 'Production', value: '1,280', trend: '-2%', icon: Activity, color: 'text-orange-600', bg: 'bg-orange-50 shadow-orange-100' },
          { label: 'ESG Score', value: '88/100', trend: '+1%', icon: Zap, color: 'text-indigo-600', bg: 'bg-indigo-50 shadow-indigo-100' },
        ].map((card, idx) => (
          <div key={idx} className="bg-white p-8 rounded-[2.5rem] shadow-lg shadow-slate-100 border border-slate-100 flex flex-col justify-between hover:scale-[1.03] transition-all cursor-default">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 bg-white p-10 rounded-[3rem] shadow-xl shadow-slate-100 border border-slate-100">
          <div className="flex items-center justify-between mb-10">
            ...
            <div className="flex gap-4">
                ...
                <span className="flex items-center gap-2 text-xs font-black uppercase tracking-widest text-slate-400">
                    <div className="w-3 h-3 rounded-full bg-emerald-400"></div> Net Profit
                </span>
            </div>
          </div>
        ...
        <div className="bg-white p-10 rounded-[3rem] shadow-xl shadow-slate-100 border border-slate-100 flex flex-col">
              ...
              {[
                ...
                <div key={idx} className="space-y-2">
                  <div className="flex justify-between text-xs font-bold uppercase tracking-tight">
                    ...
                    <span className="text-slate-400">{team.share}% Quality Score</span>
                  </div>
          ...
          <button className="w-full mt-10 py-4 bg-slate-50 text-xs font-black uppercase tracking-widest text-slate-600 hover:bg-blue-600 hover:text-white rounded-2xl transition-all flex items-center justify-center gap-2">
