          ...
          <AnimatePresence mode="wait">
            ...
            {step === 3 && (
              <motion.div key="s3" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="space-y-12">
                 ...
                 <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                    <div className="bg-slate-900/60 p-10 rounded-[3.5rem] border border-white/10 space-y-10 shadow-2xl h-fit">
                       <WizardField label="Escopo de Regiões" type="number" val={formData.regionsCount} onChange={(v:any)=>setFormData({...formData, regionsCount: Math.min(15, Math.max(1, parseInt(v)))})} />
                       <div className="p-6 bg-emerald-600/5 rounded-3xl border border-emerald-500/10">
                          <p className="text-[9px] text-slate-500 font-bold uppercase leading-relaxed italic">"Cada região demanda decisões individuais de Preço, Marketing e Prazos no Cockpit."</p>
                       </div>
                    </div>
                    <div className="space-y-4">
                       <h4 className="text-[10px] font-black text-emerald-400 uppercase tracking-[0.4em] italic flex items-center gap-3"><Boxes size={16}/> Identificadores Geográficos</h4>
                       <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-[400px] overflow-y-auto pr-4 custom-scrollbar">
                          {regionNames.map((n, i) => (
                             <input key={i} value={n} onChange={e => { const next = [...regionNames]; next[i] = e.target.value; setRegionNames(next); }} className="w-full bg-slate-900 border border-white/10 p-4 rounded-2xl text-[10px] font-black text-white uppercase outline-none focus:border-emerald-500 transition-all shadow-inner" placeholder={`Região ${i+1}`} />
                          ))}
                       </div>
                    </div>
                 </div>
