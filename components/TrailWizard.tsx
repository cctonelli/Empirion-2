...
const TrailWizard: React.FC<{ onComplete: () => void }> = ({ onComplete }) => {
  ...
  const normalizeWeights = () => {
    ...
    if (sum === 0) return;
    setRegionConfigs(prev => prev.map(r => ({ ...r, demand_weight: Math.round((r.demand_weight / sum) * 100) })));
  };

  // Fix: Explicitly casting INITIAL_FINANCIAL_TREE properties to AccountNode[] to satisfy complex literal union types in state
  ...
  const handleLaunch = async () => {
    ...
    try {
      ...
      await createChampionshipWithTeams({
        ...
        is_trial: true, market_indicators: { ...baseIndicators, dividend_percent: formData.dividend_percent },
            ...
            {step === 1 && (
              <motion.div key="s1" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="space-y-12">
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    <div className="lg:col-span-2"><WizardField label="NOME DO TORNEIO" val={formData.name} onChange={(v:any)=>setFormData({...formData, name: v})} placeholder="Ex: TORNEIO DOS CAMPEÕES" /></div>
                    ...
                    <div className="overflow-auto custom-scrollbar flex-1 relative">
                       <table className="w-full text-left border-separate border-spacing-0">
                          <thead className="sticky top-0 z-[100] bg-slate-900 shadow-xl">
                             <tr>
                                <th className="p-4 bg-slate-900 border-b-2 border-r-2 border-white/10 w-[280px] min-w-[280px]"><span className="text-[9px] font-black text-slate-400 uppercase tracking-[0.3em]">Variações / Período</span></th>
                                {Array.from({ length: totalPeriods }).map((_, i) => (
                                   <th key={i} className={`p-4 bg-slate-900 border-b-2 border-r border-white/5 text-center min-w-[95px] ${i === 0 ? 'bg-orange-600/10' : ''}`}><span className={`text-[12px] font-black uppercase tracking-widest ${i === 0 ? 'text-white' : 'text-orange-500'}`}>P{i < 10 ? `0${i}` : i}</span></th>
                                ))}
                             </tr>
                          </thead>
            ...
            {step === 6 && (
              <motion.div key="s6" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-12">
                 ...
                 <div className="relative group">
                    <div className="absolute inset-0 bg-slate-950/40 backdrop-blur-[2px] z-50 flex items-center justify-center rounded-[4rem] border-4 border-dashed border-orange-500/20">
                       <div className="bg-slate-900 p-10 rounded-[3rem] border border-white/10 shadow-3xl text-center space-y-6 max-w-md animate-in zoom-in-95">
                          <Lock size={48} className="text-orange-500 mx-auto" />
                          <h4 className="text-2xl font-black text-white uppercase italic">Protocolo Imutável</h4>
                          <p className="text-slate-400 text-sm font-medium leading-relaxed italic">
                             "A personalização de Balanços e DREs iniciais (P00) é exclusiva para usuários registrados com licença PRO."
                          </p>
                          <button className="px-8 py-3 bg-orange-600 text-white rounded-full font-black text-[10px] uppercase tracking-widest shadow-xl">Conhecer Planos</button>
                       </div>
                    </div>
                    <div className="bg-slate-950/90 p-4 rounded-[4rem] border-2 border-white/10 opacity-40 grayscale pointer-events-none">
...
const SummaryCard = ({ label, val, icon, color }: any) => (
  <div className="bg-slate-900/80 p-6 rounded-3xl border border-white/5 flex items-center gap-6 shadow-xl">
     ...
     <div><span className="block text-[8px] font-black text-slate-500 uppercase tracking-widest mb-1">{label}</span><span className="text-2xl font-black text-white font-mono italic">$ {val.toLocaleString()}</span></div>
  </div>
);
...
const CompactMatrixRow = ({ label, macroKey, rules, update, icon, periods, isExchange }: any) => (
   <tr className="hover:bg-white/[0.04] transition-colors group">
      ...
      {Array.from({ length: periods }).map((_, i) => {
         ...
         return (
            <td key={i} className={`p-1 border-r border-white/5 ${i === 0 ? 'bg-orange-600/5' : ''}`}><input type="number" step={isExchange ? "0.01" : "0.1"} value={isExchange ? val.toFixed(2) : val} onChange={e => update(i, macroKey, parseFloat(e.target.value))} className={`w-full bg-slate-900 border border-white/5 rounded-xl px-2 py-2.5 text-center text-[10px] font-black outline-none focus:border-orange-500 transition-all shadow-inner ${i === 0 ? 'border-orange-500/40 text-white' : 'text-white'}`} /></td>
         );
      })}
   </tr>
);
...
const WizardField = ({ label, val, onChange, type = 'text', placeholder }: any) => (
  <div className="space-y-3 text-left group">
     ...
     <input type={type} value={val} onChange={e => onChange(e.target.value)} className="w-full bg-slate-950 border-4 border-white/5 rounded-3xl px-8 py-5 text-lg font-bold text-white outline-none focus:border-orange-600 transition-all shadow-[inset_0_5px_15px_rgba(0,0,0,0.4)] placeholder:text-slate-800 font-mono" placeholder={placeholder} />
  </div>
);
...
const WizardSelect = ({ label, val, onChange, options }: any) => (
  <div className="space-y-3 text-left group">
     <label className="text-[10px] font-black uppercase text-slate-500 tracking-[0.4em] ml-2 group-focus-within:text-orange-500 transition-colors italic">{label}</label>
     <div className="relative"><select value={val} onChange={e => onChange(e.target.value)} className="w-full bg-slate-950 border-4 border-white/5 rounded-3xl px-8 py-5 text-[11px] font-black text-white uppercase outline-none focus:border-orange-600 transition-all cursor-pointer appearance-none shadow-[inset_0_5px_15px_rgba(0,0,0,0.4)]">{options.map((o: any) => <option key={o.v} value={o.v} className="bg-slate-900">{o.l}</option>)}</select><div className="absolute right-8 top-1/2 -translate-y-1/2 pointer-events-none text-slate-500"><Settings2 size={24} /></div></div>
  </div>
);
