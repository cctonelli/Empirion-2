...
import { 
  ...
  BarChart3, AlertOctagon, TrendingUp, HelpCircle, Flame, Users, AlertTriangle, Briefcase
} from 'lucide-react';
...
export const RightPreviewPanel: React.FC<RightPreviewPanelProps> = ({
  ...
  return (
    <div className="w-full bg-slate-950/95 border-t lg:border-t-0 lg:border-l border-white/5 shrink-0 flex flex-col h-full overflow-hidden transition-all duration-300 ease-in-out lg:w-[410px] shadow-[0_0_80px_rgba(0,0,0,0.5)] z-40 relative">
      {/* Header do Preview */}
      ...
      {/* Widgets de Simulação Instantânea do Auditor Z-Guard, E-SDS, Kardex, Finanças e Riscos */}
      <div className="flex-1 overflow-y-auto custom-scrollbar p-5 space-y-6">
        ...
        <div className={`p-4 rounded-2xl border transition-all shadow-lg ${validation.isValid ? 'bg-emerald-950/10 border-emerald-500/15' : 'bg-rose-950/10 border-rose-500/15'}`}>
          ...
          {validation.isValid ? (
            <p className="text-[9px] text-emerald-400/80 italic font-sans">Integridade contábil ativa entre DRE, DFC e Balanço Patrimonial (Z-Guard).</p>
          ) : (
        ...
        <div className="p-4 rounded-2xl border bg-slate-900 border-white/5 space-y-4 shadow-xl">
          ...
          <div className="space-y-2">
            ...
            <div className="relative w-full h-2 bg-slate-950 rounded-full overflow-hidden shadow-inner flex items-center">
              ...
              {/* Marcador de limite 2.0 (que seria 20% da barra) */}
              <div className="absolute left-[20%] top-0 bottom-0 w-1 bg-white/20" title="Limite de Alerta Geral" />
            </div>
          </div>
        ...
        {/* RECURSO EXCLUSIVO: PAINEL ANALÍTICO DE ALERTAS DE RISCO */}
        ...
        <div className="p-4 rounded-2xl border bg-slate-900 border-white/5 space-y-3.5 shadow-xl">
          <span className="text-[10px] font-black uppercase tracking-wider text-slate-200 flex items-center gap-1.5 font-sans">
        ...
        {/* WIDGET FINANÇAS */}
        <div className="p-4 rounded-2xl border bg-slate-900 border-white/5 space-y-2 shadow-xl">
          <span className="text-[10px] font-black uppercase tracking-wider text-slate-200 flex items-center gap-1.5 font-sans">
            <BarChart3 size={14} className="text-orange-500" />
            DRE & DFC Projetadas (T+1)
          </span>
          <div className="space-y-2 text-[10px] text-slate-400 font-medium font-mono">
            <div className="flex justify-between items-center py-1.5 border-b border-white/5">
              <span className="font-sans text-[9px] text-slate-300">Faturamento Bruto</span>
              <span className="font-bold text-white font-mono">{formatCurrency(finance.receita, activeArena?.currency || 'BRL')}</span>
            </div>
            <div className="flex justify-between items-center py-1.5 border-b border-white/5">
              <span className="font-sans text-[9px] text-slate-300">EBITDA Ajustado</span>
              <span className="font-bold text-orange-400 font-mono">{formatCurrency(finance.ebitda, activeArena?.currency || 'BRL')}</span>
            </div>
            <div className="flex justify-between items-center py-1.5 border-b border-white/5">
              <span className="font-sans text-[9px] text-slate-300">Lucro Líquido</span>
              <span className={`font-bold font-mono ${finance.lucroLiq >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                {formatCurrency(finance.lucroLiq, activeArena?.currency || 'BRL')}
              </span>
            </div>
            <div className="flex justify-between items-center py-1.5">
              <span className="font-sans text-[9px] text-slate-300">Caixa Final Estimado</span>
              <span className={`font-bold font-mono ${finance.cashFinal >= 20000 ? 'text-emerald-400' : 'text-rose-400'}`}>
                {formatCurrency(finance.cashFinal, activeArena?.currency || 'BRL')}
              </span>
            </div>
          </div>
        </div>
