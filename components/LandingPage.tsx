  ...
  return (
    <div className="min-h-screen bg-transparent font-sans text-slate-100 relative selection:bg-blue-500 selection:text-white">
      ...
      <section id="home" className="pt-48 pb-32 px-8 md:px-24 flex flex-col items-center text-center relative overflow-hidden">
        ...
        >
           ...
           >
              ...
              <span className="text-[10px] font-black uppercase tracking-[0.3em] text-blue-400">{t('hero.protocol')}</span>
           </motion.div>
           ...
           >
              {t('hero.title')} <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-indigo-500">{t('hero.empire')}</span>
           </h1 >
        ...
        >
           <div className="absolute top-0 left-0 right-0 h-16 bg-white/5 flex items-center justify-between px-10">
              <div className="flex items-center gap-3">
                 ...
                 <span className="text-[10px] font-black uppercase tracking-widest opacity-60 italic">{t('hero.nodeLabel')}</span>
              </div>
      ...
      <section id="features" className="py-32 px-8 md:px-24">
        <div className="max-w-7xl mx-auto space-y-24">
          <div className="space-y-4 text-center md:text-left">
             <h2 className="text-[10px] font-black uppercase tracking-[0.5em] text-blue-400">{t('features.badge')}</h2>
             <h3 className="text-5xl font-black text-white uppercase tracking-tighter">{t('features.title')}</h3>
          </div>
            ...
            {features.map((f, i) => (
              ...
              >
                 ...
                 <div className="space-y-4">
                    <h4 className="text-xl font-black text-white uppercase tracking-tight">{t(`features.items.${f.id}.title`)}</h4>
                    <p className="text-slate-500 text-sm font-medium leading-relaxed">{t(`features.items.${f.id}.description`)}</p>
                 </div>
              </motion.div>
            ))}
          </div>
      ...
      <section id="branches" className="py-32 px-8 md:px-24 bg-white/[0.02] backdrop-blur-3xl">
        <div className="max-w-7xl mx-auto space-y-24">
           <div className="flex flex-col md:flex-row md:items-end justify-between gap-12">
              <div className="space-y-4">
                 <h2 className="text-[10px] font-black uppercase tracking-[0.5em] text-blue-400">{t('branches.badge')}</h2>
                 <h3 className="text-6xl font-black text-white uppercase tracking-tighter">{t('branches.title')}</h3>
              </div>
              <p className="text-slate-400 font-medium max-w-md text-lg leading-relaxed italic">{t('branches.tagline')}</p>
           </div>
              ...
              {branchesOverview.map((b) => (
                ...
                >
                   ...
                   <div className="flex items-center gap-3 text-[10px] font-black uppercase tracking-[0.3em] text-blue-400 group-hover:text-white transition-colors">
                      {t('branches.explore')} <ChevronRight size={16} />
                   </div>
      ...
      <div className="space-y-1px bg-white/5">
        {branchesOverview.map((b) => {
          const slug = b.slug;
          const data = branchesDetailData[slug as keyof typeof branchesDetailData];
          return (
                     ...
                     <div className="space-y-8">
                        <div className="space-y-4">
                          ...
                          <div className="flex flex-wrap gap-2">
                            {data.kpis.map((k: string, idx: number) => (
      ...
      <section id="ia" className="py-40 px-8 md:px-24 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-24 items-center relative z-10">
           <div className="space-y-16">
              <div className="space-y-6">
                 <h2 className="text-[10px] font-black uppercase tracking-[0.5em] text-blue-400 italic">{t('ia.badge')}</h2>
                 ...
                 <p className="text-slate-400 text-xl font-medium leading-relaxed">{t('ia.description')}</p>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                {['reasoning', 'grounding', 'reporting', 'predictive'].map((key, i) => (
                  <div key={i} className="space-y-3 group">
                     <div className="flex items-center gap-4">
                        <div className="w-1.5 h-6 bg-blue-500 rounded-full group-hover:scale-y-125 transition-transform"></div>
                        <h5 className="font-black uppercase tracking-widest text-sm text-white">{t(`ia.items.${key}.title`)}</h5>
                     </div>
                     <p className="text-slate-500 text-xs font-bold leading-relaxed ml-6">{t(`ia.items.${key}.desc`)}</p>
                  </div>
                ))}
              </div>
           </div>
           ...
           <div className="bg-slate-900/60 backdrop-blur-3xl border border-white/5 p-16 rounded-[5rem] space-y-10 shadow-[0_50px_100px_rgba(0,0,0,0.6)] relative overflow-hidden group">
              ...
              <div className="flex items-center gap-6">
                 ...
                 <span className="font-black uppercase tracking-[0.3em] text-[12px] text-blue-400 italic">{t('ia.active')}</span>
              </div>
      ...
      <section id="community" className="py-40 px-8 md:px-24 bg-white/[0.01]">
        <div className="max-w-7xl mx-auto space-y-24">
           <div className="text-center space-y-6">
              <h2 className="text-[10px] font-black uppercase tracking-[0.5em] text-blue-400">{t('community.badge')}</h2>
              <h3 className="text-6xl font-black text-white uppercase tracking-tighter">{t('community.title')}</h3>
           ...
           <div className="grid grid-cols-1 md:grid-cols-3 gap-1px bg-white/5 border border-white/5 rounded-[4rem] overflow-hidden">
              {[
                ...
                <div key={i} className="text-center p-20 bg-slate-950/40 hover:bg-white/[0.02] transition-colors">
                   ...
                   <span className="text-[11px] font-black uppercase tracking-[0.4em] text-slate-500">{t(`community.stats.${s.key}`)}</span>
                </div>
              ))}
           </div>
        </div>
      ...
      <footer className="bg-slate-950 pt-32 pb-16 px-8 border-t border-white/5">
        <div className="max-w-7xl mx-auto">
           <div className="grid grid-cols-1 md:grid-cols-4 gap-24 mb-32">
              ...
              <div className="space-y-6">
                 <h5 className="text-[10px] font-black uppercase tracking-[0.4em] text-white/40">{t('footer.nav')}</h5>
                 <div className="flex flex-col gap-4">
                    {menuItems.map(item => <button key={item.id} onClick={() => scrollToSection(item.id)} className="text-xs font-black uppercase tracking-widest text-slate-500 hover:text-white transition-colors text-left">{t(`common:${item.id}`, { defaultValue: item.label })}</button>)}
                 </div>
              </div>
              <div className="space-y-6">
                 <h5 className="text-[10px] font-black uppercase tracking-[0.4em] text-white/40">{t('footer.legal')}</h5>
                 <div className="flex flex-col gap-4">
