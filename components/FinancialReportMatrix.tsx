...
const FinancialReportMatrix: React.FC<MatrixProps> = ({ type, history, projection, currency, startingMode }) => {
  ...
  const renderStrategicRows = () => {
    const kpiDefinitions = [
      ...
      { id: 'stock_quantities.mp_a', label: 'Estoque: MP A', suffix: ' un', desc: 'Saldo de matéria-prima A' },
      { id: 'stock_quantities.mp_b', label: 'Estoque: MP B', suffix: ' un', desc: 'Saldo de matéria-prima B' },
      { id: 'stock_quantities.finished_goods', label: 'Estoque: PA', suffix: ' un', desc: 'Saldo de produtos prontos' },
                  ...
                  <div className="flex flex-col items-center gap-0.5">
                    ...
                    <span className="text-[9px] tracking-tighter col-header-period-label">
                      {p.isProjection 
                        ...
                        : (p.round === 0 ? 'R-0 (INICIAL)' : `R-${p.round < 10 ? '0' : ''}${p.round}`)}
                    </span>
                  </div>
                </th>
              ))}
