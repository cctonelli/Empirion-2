...
import {
  ...
  copyToClipboardTSV,
  exportToExcelXLSX,
} from "../analise-gerencial/export-to-spreadsheet";
...
const FinancialReportMatrix: React.FC<MatrixProps> = ({
  type,
  history,
  projection,
  currency,
  ...
  const getTitle = () => {
    ...
    if (type === "dre")
      return "DRE - Demonstrativo de Resultados (Competência)";
    if (type === "cashflow")
      return "DFC - Fluxo de Caixa Preditivo (Regime de Caixa)";
    if (type === "commitments")
      return "Agenda de Compromissos Financeiros (Direitos e Deveres)";
    if (type === "kardex")
      return "Kardex de Estoques & Detalhamento do CPV (Absorção WAC)";
      ...
      ? [
          {
            round:
              history.length > 0
                ? (history[history.length - 1]?.round ?? 0) + 1
                : 1,
            data:
              type === "strategic" ||
              type === "kardex" ||
  ...
  const getKpiValue = (p: any, kpiId: string): any => {
    ...
    if (kpiId === "irr") {
      ...
      if (periodR00) {
        const bsR00 =
          periodR00.raw?.kpis?.statements?.balance_sheet ||
          periodR00.data?.statements?.balance_sheet;
        if (bsR00) {
          plR00 =
            findAccountValueLocal(bsR00, "equity.capital") +
            findAccountValueLocal(bsR00, "equity.profit");
        }
      }
      ...
      if (plR00 <= 0) {
        ...
        const bsR01 =
          periodR01?.raw?.kpis?.statements?.balance_sheet ||
          periodR01?.data?.statements?.balance_sheet;
        if (bsR01) {
      ...
      for (let r = 1; r <= p.round; r++) {
        ...
        if (per) {
          ...
          if (fcoLivre === undefined || fcoLivre === null) {
            const dre =
              per.raw?.kpis?.statements?.dre || per.data?.statements?.dre;
            const dfc =
              per.raw?.kpis?.statements?.cash_flow ||
              per.data?.statements?.cash_flow;
            ...
            const capex = Math.abs(
              findAccountValueLocal(dfc, "cf.outflow.maintenance"),
            );
    ...
    const kpiDefinitions = [
      {
        ...
        suffix: " dias",
        desc: "Eficiência do capital de giro",
      },
      {
        id: "interest_coverage",
        label: "Índice de Cobertura de Juros",
        suffix: "x",
        desc: "Capacidade de pagamento de juros",
      },
      {
        id: "altman_z_score",
        label: "Altman Z''-Score (Solvência)",
        suffix: "",
        desc: "Saúde financeira global (Seguro > 5.85)",
      },
      {
        id: "market_share",
        label: "Market Share Real",
        suffix: "%",
        desc: "Participação de mercado no round",
      },
      {
        id: "markup",
        label: "Markup Médio (Margem Bruta)",
        suffix: "%",
        isPercent: true,
        desc: "Margem sobre o custo unitário",
      },
      {
        id: "stock_quantities.mp_a",
        label: "Estoque: MP A",
        suffix: " un",
        desc: "Saldo de matéria-prima A",
      },
      {
        id: "stock_quantities.mp_b",
        label: "Estoque: MP B",
        suffix: " un",
        desc: "Saldo de matéria-prima B",
      },
      {
        id: "stock_quantities.finished_goods",
        label: "Estoque: PA",
        suffix: " un",
        desc: "Saldo de produtos prontos",
      },
      {
        id: "price_elasticity",
        label: "Elasticidade-Preço Real",
        suffix: "",
        desc: "Sensibilidade da demanda ao preço",
      },
      {
        id: "carbon_footprint",
        label: "Pegada de Carbono Unitária",
        suffix: " kg CO2",
        desc: "Impacto ambiental projetado",
      },
      {
        id: "dupont.margin",
        label: "Margem Líquida (DuPont)",
        suffix: "%",
        isPercent: true,
        desc: "Eficiência de lucro",
      },
      {
        id: "dupont.turnover",
        label: "Giro do Ativo (DuPont)",
        suffix: "x",
        desc: "Eficiência operacional",
      },
      {
        id: "dupont.leverage",
        label: "Alavancagem (DuPont)",
        suffix: "x",
        desc: "Multiplicador de patrimônio",
      },
      {
        id: "roe",
        label: "Retorno s/ Patrimônio Líquido (ROE)",
        suffix: "%",
        isPercent: true,
        desc: "Rentabilidade real do capital próprio (DuPont)",
      },
      {
        id: "bep",
        label: "Ponto de Equilíbrio (Break-Even)",
        suffix: "",
        isCurrency: true,
        desc: "Faturamento mínimo operacional exigido para lucro operacional zero",
      },
      {
        id: "irr",
        label: "Taxa Interna de Retorno (TIR / IRR)",
        suffix: "%",
        isPercent: true,
        desc: "Retorno econômico acumulado baseado no PL de abertura",
      },
    ];
      ...
      >
        <td className="p-1.5 sticky left-0 bg-slate-900 z-30 border-r border-white/10 min-w-[200px] shadow-xl">
          <div className="flex flex-col">
            <span className="text-[10px] uppercase tracking-[0.15em] text-white font-black group-hover:text-orange-500 transition-colors">
              {kpi.label}
            </span>
            <span className="text-[9px] text-slate-500 uppercase tracking-widest mt-0.5 italic font-medium">
              {kpi.desc}
            </span>
          </div>
        </td>
        {periods.map((p: any, idx) => {
          ...
          const prevVal =
            idx > 0 ? getKpiValue(periods[idx - 1], kpi.id) : undefined;
          ...
          return (
            <td
              key={idx}
              className={`p-1.5 text-center font-mono text-[12px] ${p.isProjection ? "bg-orange-600/5" : ""} ${colorClass}`}
            >
              <div className="flex flex-col items-center gap-0.5">
                <span className="text-base tracking-tighter font-bold">
                  {displayVal}
                  {val !== undefined &&
                  val !== null &&
                  val !== "N/A" &&
  ...
  const renderCommitmentRows = () => {
    const receivablesSchema = [
      { id: "clients", label: "Contas a Receber" },
      { id: "investments", label: "Aplicações Financeiras" },
      { id: "vat_recoverable", label: "IVA a Recuperar" },
    ];

    const payablesSchema = [
      { id: "suppliers", label: "Fornecedores" },
      { id: "loans_st", label: "Empréstimos (Curto Prazo)" },
      { id: "loans_lt", label: "Empréstimos (Longo Prazo)" },
      { id: "taxes", label: "Imposto de Renda a Pagar" },
      ...
      <>
        <tr className="bg-emerald-500/10 font-black">
          <td
            ...
            className="p-2 text-emerald-400 text-[9px] uppercase tracking-widest"
          >
        ...
        {receivablesSchema.map((item: any) => (
          ...
          >
            ...
            {periods.map((p: any, idx: number) => {
              const val =
                p.data?.commitments?.receivables?.find(
                  (r: any) => r.id === item.id,
                )?.value || 0;
              return (
                <td
                  key={idx}
                  className={`p-1.5 text-center font-mono text-[12px] ${p.isProjection ? "bg-orange-600/5 text-orange-500 font-black" : "text-slate-300"}`}
                >
        ...
        <tr className="bg-rose-500/10 font-black">
          <td
            colSpan={periods.length + 1}
            className="p-2 text-rose-400 text-[9px] uppercase tracking-widest"
          >
        ...
        {payablesSchema.map((item: any) => (
          ...
          >
            ...
            {periods.map((p: any, idx: number) => {
              let val =
                p.data?.commitments?.payables?.find(
                  (r: any) => r.id === item.id,
                )?.value || 0;

              // Fallback dinâmico sutil para cálculo reativo imediato de juros (interests) com base nas despesas financeiras do período/dre
              if (item.id === "interests" && val === 0) {
                const dreList = p.data?.statements?.dre || [];
                const finExpNode = dreList.find(
                  (node: any) =>
                    node.id === "fin.exp" || node.id === "financial_expenses",
                );
                if (finExpNode) {
                  val = Math.abs(Number(finExpNode.value) || 0);
                }
              }
        ...
        {!periods[periods.length - 1]?.data?.loans ||
        periods[periods.length - 1]?.data?.loans.length === 0 ? (
          ...
          periods[periods.length - 1]?.data?.loans?.map(
            (loan: any, lIdx: number) => {
              ...
              const sch = periods[
                periods.length - 1
              ]?.data?.amortization_schedule?.find(
                (s: any) => s.id === loan.id,
              );
              return (
                                ...
                                <div className="text-[9px] text-slate-400 leading-normal mt-1 font-mono space-y-0.5">
                                  <div>
                                    ...
                                    <span className="text-slate-300 font-semibold">
                                      {formatCurrency(inst.amort, currency)}
                                    </span>
                                  </div>
                                  <div>
                                    Juros:{" "}
        ...
        <tr className="bg-rose-500/5 border-b border-white/5">
          <td colSpan={periods.length + 1} className="p-2.5 pl-3">
            <span className="text-[8px] text-rose-300 italic flex items-center gap-1.5 font-sans leading-normal">
              ...
              saudáveis de caixa e de geração de EBITDA é crucial para evitar
              penalização do rating corporativo.
            </span>
          </td>
        </tr>
      </>
  ...
  const renderKardexRows = () => {
    const rows = [
      // Seção MP A
      {
        section: "Matéria-Prima A (Fluxo Físico e Financeiro WAC)",
        key: "mpa.saldoInicialQtd",
        label: "(=) Estoque Inicial (Qtd)",
        desc: "Saldo inicial de MP A",
        formatter: (v: number) =>
          typeof v === "number" ? v.toLocaleString("pt-BR") + " un" : "0 un",
      },
      {
        key: "mpa.saldoInicialValor",
        label: "(=) Estoque Inicial ($)",
        desc: "Saldo liquefeito inicial de MP A",
        formatter: (v: number) => formatCurrency(v || 0, currency),
      },
      {
        key: "mpa.entradasQtd",
        label: "(+) Compras (Normais/Especiais) (Qtd)",
        desc: "Compras totais de MP A no round",
        formatter: (v: number) =>
          typeof v === "number" ? v.toLocaleString("pt-BR") + " un" : "0 un",
      },
      {
        key: "mpa.entradasValor",
        label: "(+) Compras (Normais/Especiais) ($)",
        desc: "Valor gerador líquido das compras de MP A",
        formatter: (v: number) => formatCurrency(v || 0, currency),
      },
      {
        key: "mpa.saidasQtd",
        label: "(-) Consumo Produção (Qtd)",
        desc: "Consumo no Trial Industrial (Produção * 3)",
        formatter: (v: number) =>
          typeof v === "number" ? v.toLocaleString("pt-BR") + " un" : "0 un",
      },
      {
        ...
        desc: "Valorização de consumo industrial ponderado",
        formatter: (v: number) => formatCurrency(v || 0, currency),
      },
      {
        key: "mpa.saldoFinalQtd",
        label: "(=) Estoque Final (Qtd)",
        desc: "Quantidade de fechamento físico de MP A",
        formatter: (v: number) =>
          typeof v === "number" ? v.toLocaleString("pt-BR") + " un" : "0 un",
      },
      {
        key: "mpa.saldoFinalValor",
        label: "(=) Estoque Final ($)",
        desc: "Valor total do estoque de fechamento",
        formatter: (v: number) => formatCurrency(v || 0, currency),
      },
      {
        key: "mpa.saldoFinalUnitario",
        label: "Preço Médio Ponderado WAC",
        desc: "Preço interno de custeio médio para MP A",
        formatter: (v: number) => formatCurrency(v || 0, currency) + "/un",
      },

      // Seção MP B
      {
        section: "Matéria-Prima B (Fluxo Físico e Financeiro WAC)",
        key: "mpb.saldoInicialQtd",
        label: "Estoque Inicial (Qtd)",
        desc: "Saldo inicial de MP B",
        formatter: (v: number) =>
          typeof v === "number" ? v.toLocaleString("pt-BR") + " un" : "0 un",
      },
      {
        key: "mpb.saldoInicialValor",
        label: "(=) Estoque Inicial ($)",
        desc: "Saldo liquefeito inicial de MP B",
        formatter: (v: number) => formatCurrency(v || 0, currency),
      },
      {
        key: "mpb.entradasQtd",
        label: "(+) Compras (Normais/Especiais) (Qtd)",
        desc: "Compras totais de MP B no round",
        formatter: (v: number) =>
          typeof v === "number" ? v.toLocaleString("pt-BR") + " un" : "0 un",
      },
      {
        key: "mpb.entradasValor",
        label: "(+) Compras (Normais/Especiais) ($)",
        desc: "Valor gerador líquido das compras de MP B",
        formatter: (v: number) => formatCurrency(v || 0, currency),
      },
      {
        key: "mpb.saidasQtd",
        label: "(-) Consumo Produção (Qtd)",
        desc: "Consumo no Trial Industrial (Produção * 2)",
        formatter: (v: number) =>
          typeof v === "number" ? v.toLocaleString("pt-BR") + " un" : "0 un",
      },
      {
      ...
      {
        key: "mpb.saldoFinalQtd",
        label: "(=) Estoque Final (Qtd)",
        desc: "Quantidade de fechamento físico de MP B",
        formatter: (v: number) =>
          typeof v === "number" ? v.toLocaleString("pt-BR") + " un" : "0 un",
      },
      {
        key: "mpb.saldoFinalValor",
        label: "(=) Estoque Final ($)",
        desc: "Valor total do estoque de fechamento",
        formatter: (v: number) => formatCurrency(v || 0, currency),
      },
      {
        key: "mpb.saldoFinalUnitario",
        label: "Preço Médio Ponderado WAC",
        desc: "Preço interno de custeio médio para MP B",
        formatter: (v: number) => formatCurrency(v || 0, currency) + "/un",
      },

      // Seção PA
      {
        section: "Produtos Acabados (Movimentação e Custo Comercial WAC)",
        key: "pa.saldoInicialQtd",
        label: "Estoque Inicial PA (Qtd)",
        desc: "Quantidade inicial de produtos acabados",
        formatter: (v: number) =>
          typeof v === "number" ? v.toLocaleString("pt-BR") + " un" : "0 un",
      },
      {
        key: "pa.saldoInicialValor",
        label: "(=) Estoque Inicial PA ($)",
        desc: "Valor do estoque pronto de abertura",
        formatter: (v: number) => formatCurrency(v || 0, currency),
      },
      {
        key: "pa.entradasQtd",
        label: "(+) Entradas Produção (Qtd)",
        desc: "Unidades finalizadas na planta fabril",
        formatter: (v: number) =>
          typeof v === "number" ? v.toLocaleString("pt-BR") + " un" : "0 un",
      },
      {
        key: "pa.entradasValor",
        label: "(+) Entradas Produção ($/CPP)",
        desc: "Custo industrial total das unidades manufaturadas",
        formatter: (v: number) => formatCurrency(v || 0, currency),
      },
      {
        key: "pa.saidasQtd",
        label: "(-) Saídas Vendas / CPV (Qtd)",
        desc: "Unidades vendidas e faturadas com sucesso",
        formatter: (v: number) =>
          typeof v === "number" ? v.toLocaleString("pt-BR") + " un" : "0 un",
      },
      {
        key: "pa.saidasValor",
        label: "(-) Saídas Vendas / CPV ($)",
        desc: "Custo faturado do produto vendido para o DRE",
        formatter: (v: number) => formatCurrency(v || 0, currency),
      },
      {
        key: "pa.saldoFinalQtd",
        label: "(=) Estoque Final PA (Qtd)",
        desc: "Quantidade remanescente de produtos em estoque",
        formatter: (v: number) =>
          typeof v === "number" ? v.toLocaleString("pt-BR") + " un" : "0 un",
      },
      {
        key: "pa.saldoFinalValor",
        label: "(=) Estoque Final PA ($)",
        desc: "Valor contábil do estoque fabril de fechamento",
        formatter: (v: number) => formatCurrency(v || 0, currency),
      },
      {
        key: "pa.saldoFinalUnitario",
        label: "Custo Unitário de Fechamento WAC",
        desc: "Custo de faturamento médio final ponderado",
        formatter: (v: number) => formatCurrency(v || 0, currency) + "/un",
      },

      // Detalhes do CPV de Absorção Industrial
      {
        section: "Demonstrativo de Formação do CPV (Absorção Industrial)",
        key: "cpv.mpConsumida",
        label: "Matéria-Prima Consumida",
        desc: "Custo de MP A e B aplicadas no processo",
        formatter: (v: number) => formatCurrency(v || 0, currency),
      },
      {
        key: "cpv.maoDeObraDireta",
        label: "Mão de Obra Direta (MOD)",
        desc: "Salários e encargos dos operadores industriais",
        formatter: (v: number) => formatCurrency(v || 0, currency),
      },
      {
        key: "cpv.depreciacaoFabril",
        label: "Depreciação Fabril",
        desc: "Rateio de depreciação de máquinas e prédios",
        formatter: (v: number) => formatCurrency(v || 0, currency),
      },
      {
        key: "cpv.manutencaoFabril",
        label: "Manutenção de Máquinas",
        desc: "Atividades corretivas de manutenção produtiva",
        formatter: (v: number) => formatCurrency(v || 0, currency),
      },
      {
        key: "cpv.indenizacoesRescisorias",
        label: "Indenizações Rescisórias",
        desc: "Custos trabalhistas rescisórios no round",
        formatter: (v: number) => formatCurrency(v || 0, currency),
      },
      {
        key: "cpv.pprProporcional",
        label: "Provisão de PPR Proporcional",
        desc: "Rateio do prêmio por resultados dos desligamentos",
        formatter: (v: number) => formatCurrency(v || 0, currency),
      },
      {
        key: "cpv.totalCPP",
      ...
      {
        key: "cpv.custoUnitarioProducao",
        label: "Custo Unitário de PA",
        desc: "Custo unitário do Produto Acabado",
        formatter: (v: number) => formatCurrency(v || 0, currency),
      },
      {
        key: "cpv.totalCPV",
        label: "(=) CPV-CUSTO PRODUTO VENDIDO",
        desc: "Custo direcionado para confrontação de receita no DRE",
        formatter: (v: number) => formatCurrency(v || 0, currency),
        isClass:
          "bg-orange-950/20 text-orange-400 font-black border-y border-orange-500/20",
      },
    ];
            ...
            <React.Fragment key={idx1}>
              {isSectionHeader && (
                <tr className="bg-slate-900/40 border-y border-white/5">
                  <td
                    colSpan={periods.length + 1}
                    className="p-2 text-[8px] font-black tracking-widest text-orange-400 uppercase"
                  >
                    {row.section}
                  </td>
                </tr>
              ...
              >
                <td className="p-1.5 sticky left-0 bg-slate-900 z-30 border-r border-white/10 min-w-[200px] shadow-xl">
                  <div className="flex flex-col">
                    ...
                    <span className="text-[8px] text-slate-500 uppercase tracking-widest mt-0.5 italic">
                      {row.desc}
                    </span>
                  </div>
                </td>
                {periods.map((p: any, idx2) => {
                  ...
                  let targetKardex =
                    kpis.kardex || kpis.statements?.kardex || {};
                  let targetCpv =
                    kpis.cpv_details || kpis.statements?.cpv_details || {};
                  ...
                  if (val === undefined || val === null) {
                    console.warn(
                      `Kardex/CPV missing → key: ${row.key} | round: ${p.round}`,
                    );
                    val = 0;
                  }

                  const displayStr = row.formatter
                    ? row.formatter(val)
                    : String(val);
                  ...
                  return (
                    <td
                      key={idx2}
                      className={`p-1.5 text-center font-mono text-[11px] ${p.isProjection ? "bg-orange-600/5 text-orange-400 font-extrabold" : "text-slate-300"}`}
                    >
                      {displayStr}
                    </td>
                  );
          ...
          >
            <td className="p-3 sticky left-0 bg-slate-900 z-30 border-r border-white/10 min-w-[280px] shadow-xl">
              ...
              >
                ...
                <span
                  className={`text-[12px] uppercase tracking-[0.1em] transition-colors ${isTotalizer ? "text-white group-hover:text-orange-500" : "text-slate-400 group-hover:text-slate-200"}`}
                >
                  {node.label}
                </span>
              </div>
            ...
            {periods.map((p: any, idx) => {
              ...
              return (
                <td
                  key={idx}
                  className={`p-3 text-center font-mono text-sm ${p.isProjection ? "bg-orange-600/5 text-orange-500 font-black" : "text-slate-300"}`}
                >
    ...
    <div className="flex flex-col bg-slate-950 border border-white/10 rounded-xl overflow-hidden shadow-[0_0_100px_rgba(0,0,0,0.5)]">
      <header className="p-2 bg-slate-900/80 backdrop-blur-xl border-b border-white/10 flex justify-between items-center shrink-0 z-50">
        <div className="flex items-center gap-3">
          <div className="p-1.5 bg-white/5 rounded-lg shadow-inner border border-white/5">
            {getIcon()}
          </div>
          <div>
            ...
            <p className="text-[7px] text-slate-500 font-black uppercase tracking-[0.5em] mt-0.5 italic">
              Oracle High Fidelity Reporting Engine • v19.5 Sapphire
            </p>
          </div>
        </div>
              ...
              <>
                ...
                <div className="absolute right-0 mt-2 w-64 bg-slate-900 border border-white/10 rounded-xl shadow-[0_10px_30px_rgba(0,0,0,0.8)] z-50 overflow-hidden shrink-0 animate-in fade-in slide-in-from-top-2 duration-150">
                  <div className="p-2 border-b border-white/5 bg-slate-950/45">
                    <span className="text-[7px] font-black uppercase tracking-widest text-slate-500 block">
                      Opções para Matriz Financeira
                    </span>
                  </div>
                  <div className="p-1 flex flex-col gap-0.5">
                    ...
                    >
                      ...
                      <div className="flex flex-col">
                        ...
                        <span className="text-[6px] text-slate-500 font-normal normal-case">
                          Delimitador ponto-e-vírgula para Excel PT-BR
                        </span>
                      </div>
                    </button>
                    ...
                    >
                      ...
                      <div className="flex flex-col">
                        ...
                        <span className="text-[6px] text-slate-500 font-normal normal-case">
                          Copiar dados tabulados para colar direto com Ctrl+V
                        </span>
                      </div>
                    </button>
                    ...
                    >
                      ...
                      <div className="flex flex-col">
                        ...
                        <span className="text-[6px] text-emerald-500/70 font-black tracking-widest">
        ...
        <table className="w-full text-left border-collapse">
          <thead className="sticky top-0 z-40 bg-slate-900/95 backdrop-blur-md shadow-2xl">
            <tr className="text-[7px] font-black uppercase text-slate-500 tracking-[0.1em]">
              <th className="p-1.5 sticky left-0 bg-slate-900 z-50 border-r border-white/10 shadow-xl">
                Contas Contábeis de Movimento
              </th>
              {periods.map((p: any, i) => (
                <th
                  key={i}
                  className={`p-1.5 text-center border-r border-white/5 ${p.isProjection ? "bg-orange-600/10 text-orange-500" : ""}`}
                >
                  <div className="flex flex-col items-center gap-0.5">
                    <span className="opacity-50 text-[5px]">
                      {p.isProjection ? "PRÓXIMO CICLO" : "HISTÓRICO"}
                    </span>
      ...
      <footer className="p-2 bg-slate-900/90 backdrop-blur-md border-t border-white/5 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[7px] font-black uppercase tracking-[0.1em] text-slate-400 leading-relaxed">
            Consistência tripla validada com controle de WAC no Trial
            Industrial, garantindo precisão inexorável de estoque.
          </span>
        </div>
        <div className="flex items-center gap-2">
