...
1. **Modo Tradicional (Start with Base / Start with Running):**
   - As empresas herdam um balanço histórico consolidado (`INITIAL_MACHINES_R0`) contendo **5 máquinas Alpha** operacionais herdadas do Round 0.
...
2. **Receita Líquida Regional ($R_L$):**
   ...
   *(Receita Bruta deduzida do imposto IVA faturado na própria região).*
...
6. **(=) Lucro de Contribuição Regional ($LC_{\text{regional}}$):**
   ...
   *Este indicador revela a real eficiência física e comercial de precificação, marketing e penetração de cada região, variando livremente de acordo com as campanhas e logística locais.*
7. **(-) Despesas Holding Rateadas ($DCR_{\text{regional}}$):**
    As despesas corporativas que não são associáveis diretamente às regiões (como Folha Corporativa ADM/Vendas, PECLD, P&D, Resultado Financeiro, Resultado Não Operacional e Tributos Consolidados sobre a Renda) são rateadas fiduciariamente com base na representatividade de faturamento líquido de cada região:
...
### 💱 Governança Cambial: Tratamento de Moedas Locais de Regiões de Exportação (CPC 02 / IAS 21)
...
- **Digitação Nominal Direta:** As equipes devem digitar o Preço Unitário de Venda no cockpit (`MarketingStep`) diretamente na **moeda nominal local** correspondente exibida pelo campo e sugerida para aquela região de exportação (ex: digitar `120` se o preço nominal sugerido pelo mercado local for `USD 120,00`).
...
- **Modelagem de Rateio Fiduciário Reverso (FRAE v2 - Align):** Para assegurar que o mini-DRE de cada região no cockpit no *MarketingStep* exiba dados nominalmente coerentes com a moeda daquela região (ex: tudo em USD para a região americana):
...
Com o objetivo de dotar a ferramenta de alta flexibilidade durante períodos de teste acadêmico e de rigores normativos estritos durante competições corporativas reais, o controle de visibilidade das formulações de balanço e simulação P0 (`r0_templates`) opera sob uma abordagem de transição de governança:
...
- **Implementação Técnica:**
  ...
  - **Higienização de LocalStorage:** Rotinas dinâmicas no frontend varrem o e-id local gravado em cache (`local_r0_templates`) e barram do fluxo todos os itens que não possuam autoria (`tutor_id`) coincidente com a sessão ativa.
...
O fluxo econômico do fechamento do round opera em duas rodadas iterativas sintonizadas ao longo das regiões geográficas:
