...
export const calculateProjections = (
  ...
  // non_op_res pode ser injetado por auditoria (prêmios) ou deságio de venda de máquinas
  const finRes = -2500; // Proxy fixa para P00
  const nonOpRes = team.kpis?.statements?.dre?.non_op_res || 0; 
  ...
  // PPR (Participação no Lucro) - Calculado sobre o lucro após IR
  const pprRate = sanitize(decision.hr?.participationPercent, 0) / 100;
  const ppr = profitAfterTax > 0 ? profitAfterTax * pprRate : 0;
